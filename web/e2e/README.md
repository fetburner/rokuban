...
- ④ テキスト条件に打って「検索」を押した後、件数行（`N 件（番組 ID 順）`）が
  ...
  検索 API は `{site, programId}` の行だけを返すので、`page.route` のスタブは
  `POST …/programs/search` と `GET …/programs/{programId}` の 2 本で足りる
...
ため、単一サイトかつ下書きが空のスタブでは判定対象自体が存在しなかった（他の 3 本の e2e フィクスチャは
