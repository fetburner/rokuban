...
- ③ ①②はどちらもデスクトップ（1280px）でも走る（`checkViewport` はビューポート
  を変えて呼ぶだけ）。ただし①のボトムタブとの重なり判定は 1280px ではボトム
  タブ自体が無い（`md:hidden`）ため矩形が取れずスキップされ、実質②だけの
  確認になる --- issue 本文が「デスクトップではボタンは見えるが、キーワードは
  同じく一段奥」と言っているため
