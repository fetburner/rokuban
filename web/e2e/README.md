...
**選択肢の出所を先頭サイトから全 site の union に変えた現在の実測は①が 0、
②が 0.00066。** `ServiceFields` の位置と非同期／同期の境界（上記）は
...
解決した `GET /api/sites` のキャッシュを再利用する同期的な節で、かつ site が
2 つ以上のときしか描画しない --- このフィクスチャは単一サイトなので DOM に
一切増えない。
...
サイトチップは site が 2 つ以上のときしか描画しないため、単一サイトの
スタブでは判定対象自体が存在しなかった（他の 3 本の e2e フィクスチャは
単一サイトのままなので、サイトチップの実ブラウザ判定はここが唯一）。
