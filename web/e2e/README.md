...
390px 幅で番組リストを開き、ドキュメント最下端までスクロールすると、末尾の行が
ボトムタブに半分だけ隠れる（issue #303）。原因は `--bottom-nav-height`
（`web/src/index.css`）がボトムタブの実際の描画高さ（`border-t` 1px を含む）と
1px 食い違っており、`main` の `padding-bottom` がタブの実寸に足りていなかったこと。
jsdom は `getBoundingClientRect` を計算しない（常に 0 を返す）ため、この重なりは
原理的にユニットテストでは検出できない。

**初回表示（スクロール前）でも同じ重なりが起きうる**が、これはページ全体スクロール +
`fixed` なタブという今のレイアウトが持つ一般的な性質で、単一のスクロール位置の
補正では直せない（先頭行の日付ヘッダへの食い込みという同じ形の欠陥が反対の端に
移るだけだった。詳細は `docs/frontend/scroll.md`「ボトムタブの裏に隠れる行」）。この
スクリプトは、実際に直した原因（`--bottom-nav-height` とタブの実測高さの食い違い）
だけを見る。

見るのは:

- ① `--bottom-nav-height` がボトムタブの実際の描画高さ（border 込み）と一致すること
  --- 直した原因そのものの確認
- ② ドキュメント最下端まで実際にスクロールしても重ならないこと。余白が 0px
  ちょうどで通っているだけの脆いアサーションにせず、最終行の下端からタブ上端
  までの距離が負でないことも直接見る
