...
- ④ 折りたたみ行の操作列を実座標へ `page.touchscreen.tap()` で生タップ
  ...
  `overflow-hidden` は列そのものを畳むので飛ばない
