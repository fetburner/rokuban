...
// 一致しているかの受け入れ判定。
//
// **この判定は途中のスクロール位置で行がボトムタブの裏に入らないことを見ていない。**
// タブは `fixed` のオーバーレイのままとし、保証するのはドキュメント最下端まで
// スクロールしたときに末行を完全に読める到達可能性である（詳細は
...
//      のに行が絶対配置でせり出す等）を捉える回帰確認として置いてある。
//
 ...
 * なる --- 密度が薄いと 1 画面に収まってしまい、最下端でのタブとの関係を
 * 測れない。
 */
