...
//      ホーム新設（M8-3）前は `/?at=...` だった。`view` の URL 化は issue #437）
//      へ飛ぶこと --- バッジが行本体の `<a>` の中に入れ子の `<a>` として置かれて
//      いると、クリックの宛先が不定になり、多くのブラウザ実装では外側（詳細ページ）が
...
//   ② 遷移後、`lg` 以上の画面幅ではリンクが積んだ `view=grid` どおり初回レンダー
//      からグリッド表示になり、不足区間の帯（`[data-testid="capacity-band"]`）の
//      上辺が実際に画面内（スクロール済みの可視範囲）に入っていること --- スクロール
//      位置は jsdom が原理的に測れない領域（`getBoundingClientRect()` が常に 0 を
//      返す）ので、これが唯一の判定手段になる
...
if ((await badge.count()) > 0) {
  ...
  try {
    ...
    if (url.pathname === '/reservations' || /^\/reservations\//.test(url.pathname)) {
      ...
      // バッジは `view: 'grid'` を明示して積む（issue #437）。`at` だけで
      // `useMediaQuery` から推論する旧実装ではないので、`view` が URL に無い
      // ならこの導線が壊れている。
      ng.push('① 番組表へ遷移したが URL に view が付いていない（view=grid の明示が壊れている）')
    } else {
