...
//
// **ホーム（`/`）はここでは見ない。** ラボ計測はホームのデスクトップでも
// 0.111（スロットル時のみ）を報告しているが、原因は「4 セクションの表示順は
// 固定なのに解決順は不定で、後続セクションが先に見えている状態で先行セクションが
// 実データごと上に挿し込まれる」形で、対策には `docs/frontend/home.md`
// 「セクションの可視性は個別に」を変える設計判断が要る。判定手段（この形の
// フィクスチャ）ごとその判断のあとに足す。
//
