 ...
 * 常に全件を引く設計なので一覧はどの画面でも温まっている前提に乗れ、録画の
 * 展開ごとに個別の 1 件取得を増やす理由がない。**キャッシュが空の状態で
 * 複数の `RecordingDetail` が同一コミットでマウントされた場合**（同じ
 * `queryKey` の 2 つ目以降の観測者が、1 つ目の未解決フェッチに相乗りする）
 * は取得が 1 回にまとまることを `recordings.test.tsx`
 * 「ruleId を持つ複数の行を同一コミットで展開すると /api/rules の取得は
 * 1 回にまとまる（react-query の初回フェッチの重複排除）」で固定した。
 * 逆に、片方が解決してキャッシュに乗った後で別の行を展開すると、既定の
 * `staleTime: 0` によりマウントごとに refetch が起きる（同テストで
 * `userEvent.click` を逐次 await する書き方を最初に試して 2 回になることを
 * 確認済み）--- 「常に 1 回にまとまる」わけではない。
 *
 ...
 * セクションごと消える（`#N` へは落ちない）。`#N` に落ちるのは、一覧クエリが
 * 未解決（初回展開時は `#N` からルール名へ表示が切り替わる）・取得失敗・
 * キャッシュが古くて（新しく作られたルールがまだ乗っていない等）該当 id が
 * 見つからない、という一時的な状態。
 *
