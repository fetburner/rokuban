...
export function SearchPage() {
  ...
  return (
    <>
      ...
      >
        <TextMatchFields draft={draft} onChange={setDraft} />
        <ServiceFields
          draft={draft}
          ...
          onChange={setDraft}
        />
        <ChannelTypeFields draft={draft} onChange={setDraft} />
        <GenreFields draft={draft} onChange={setDraft} />
        <TimeWindowFields draft={draft} onChange={setDraft} />
        <ScalarFields draft={draft} onChange={setDraft} />
...
function TextMatchFields({ draft, onChange }: FieldsProps) {
          ...
          {draft.textMatches.map((match, index) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Chip active={match.negate} onClick={() => update(index, { negate: !match.negate })}>
                  除外
                </Chip>
              </div>
              {match.mode === 'regex' && (
...
function ChannelTypeFields({ draft, onChange }: FieldsProps) {
  return (
...
function GenreFields({ draft, onChange }: FieldsProps) {
  return (
    <Section title="ジャンル">
...
function TimeWindowFields({ draft, onChange }: FieldsProps) {
...
function ScalarFields({ draft, onChange }: FieldsProps) {
  return (
