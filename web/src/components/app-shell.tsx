  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <StickyBanners />
        {/* モバイルではボトムタブの実寸ぶんを下に確保し、ドキュメント最下端まで
            スクロールしたときに末行を完全に読める到達可能性を保証する。タブは
            fixed のオーバーレイのままなので、途中のスクロール位置で行が裏に入る
            ことは仕様として受け入れる（docs/frontend/scroll.md「ボトムタブの裏に
            隠れる行」）。 */}
        <main
          id="main"
          tabIndex={-1}
