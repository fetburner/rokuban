...
import type { LiveDiagnostics, LiveLoadError, StallHandling, StallTracker } from '@/lib/live'
import {
...
type LivePlayerProps = {
   ...
   * master playlist かどうかで判断する。`lib/live.ts` の `isMasterPlaylist`）。
...
export function LivePlayer({
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      // 自動降格を試してよいか（issue #871）。**master playlist では試さない。**
      // そのとき streamer は `?profile=` に関わらず同じ master を返すので
      // （`live.captions: true`）、下げても何も変わらないのに「下げました」と
      // 表示することになる。判定は probe が読んだ本文に基づく
      //（`lib/live.ts` の `isMasterPlaylist` --- API の一覧ではなく本文が権威）。
      //
      ...
      const canDowngrade = !probe.masterPlaylist && !isChase
