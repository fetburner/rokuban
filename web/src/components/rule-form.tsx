...
export function RuleEditForm({
  ...
  const saveAsNew = () => {
    if (blocked) return
    // `rules.name` に一意制約は無く（rules テーブル定義）、同名のルールは一覧側で
    // `#<id>` を添えて区別できる。それでもコピー元と意図的に別のルールであることを
    // 通常の表示だけで読めるよう、押した時点で名前が元のルールと同じままなら
    // `〜 のコピー` を付ける。名前欄そのものは書き換えない（上書き保存に戻った
    // ときに元の名前のままであってほしいため）。ユーザーが既に名前を変えているなら、
