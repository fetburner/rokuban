...
- ごみ箱ビュー = `deleted_at IS NOT NULL` の一覧。**復元は `deleted_at` を消し、`purge_requested` を下ろすだけ**（ファイル操作ゼロ・即時）。「今すぐ完全削除」も個別/一括で可能
