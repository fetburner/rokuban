...
「原本を入力とする実行中・再試行中のジョブがない」という条件は持たない。encode/thumbnail ジョブは desired なプロファイルにしか積まれないため、出力未コミットの間は上記 2 が原本を守る。出力コミット後のジョブは各ワーカー冒頭の冪等チェックが原本を開かずに skip するので、キューの状態を見る条件は上記 2 に対して冗長だった。
