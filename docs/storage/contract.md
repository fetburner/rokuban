...
- **`media_dir` 配下に、録画の実体を指すリンクを作らない（symlink / hard link）。** `rel_path` はファイルの identity なので、symlink は rescue の走査と `inplace.Register` が弾く一方、hard link は regular file と区別できず、同じ実体に live な録画が 2 行並ぶ。`(dev, ino)` による検出はスキャンをまたぐと inode がバックアップ復元で変わるため実装しない
- **ディレクトリへの symlink も作らない。** `filepath.WalkDir` は symlink を辿らないため、`media_dir` 配下の中身は rescue からエラーや警告なしに見えず、台帳にないファイルとして孤児回収の対象になり、[storage.md](../storage.md) §8 の孤児エイジング（約 2 週間）後に実削除される
- **`media_dir` 自身が symlink であることは許す。** Kubernetes のマウントや `/var/lib/rokuban/media -> /mnt/disk1/media` のような構成は通常の運用なので、配下のリンクとは区別する
...
- **前置前に ingest 済みの既存行は移行しない。** 新規 ingest 分だけ `sites/{site}/` が付き、ディスク上は前置あり/なしが混在する。これは「既存行が `sites/` から始まっていない」という前提の上に成り立つ想定であり、`sites/` 自体を先頭成分に使っていた既存の `filename_template` があれば話は別（`sites/` は `catalog/` / `thumbnails/` と同じ「新設の予約ディレクトリが過去の運用と衝突しないことを祈る」という一般的なトレードオフを負っている）
- **`rel_path` の `sites/{site}/` 前置をパースする読者がいる。** rescue のストレージスキャン（catalog が 1 世代も無いときのフォールバック）は、前置ありのファイルを前置の site で復元し、前置なしのファイルだけ `--site` にフォールバックする（[rescue.md](rescue.md)）。上記の前提が崩れていなければ混在は無害のまま保たれる --- 前置なしの行は前置導入前の単一 site 時代の ingest なので `--site` に単一の site 名を渡す運用で正しく復元でき、前置ありの行は前置自体が site を持つので `--site` の値に関わらず正しく復元できる。**ただし前提が崩れていれば結果も変わる**: 前置導入前から `sites/anime/x.m2ts` のような `filename_template` を使っていた既存行があると、rescue はそれを site 名 `anime` の前置と誤認し、site `anime` の録画として復元する（前置導入前は読み手がいなかったので無害だった偶然の一致が、rescue が前置を読むようになったことで意味を持ってしまう）
