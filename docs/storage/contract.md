...
3. **「公開済み」の定義は rename ではなく DB 登録**: コピー完了 → `media_assets` 行の登録、が公開の定義。読み手は DB に載っているパスしか見ない。rename のアトミック性に依存しないので S3 マウントの弱い意味論が許容範囲に入る。書きかけ・コピー失敗の残骸は cleanup ジョブが「DB に対応行のないファイル」として掃除する。
   ...
   **ingest のコピー完了には永続化確認まで含める**: mirakc の record pull は宛先へ直接書く例外だが、転送後に宛先ファイルの `fsync` → `Close` → 新規ファイルの直近の親ディレクトリの `fsync` をこの順に行い、全て成功してから `media_assets` を登録する。`MkdirAll` で作る階層のうち、ここで契約するディレクトリ同期は直近の親までとする。いずれかが失敗したら登録せず、エッジの record を保持して再試行する
