...
- **`media_dir` 自身も symlink にしない（実ディレクトリを指定する）。** rescue（`filepath.WalkDir`）と孤児回収（`filepath.Walk`）はどちらも root を lstat するため、root が symlink だと配下に一切降りない。rescue は 0 件のまま「成功」し、孤児回収の全損セーフガード（走査結果が 0 件なら記録を見送る）も働かないため、active な行が全件「実体無し」と誤検知され続ける。Kubernetes のマウントは bind mount で実ディレクトリなので影響しない。起動時に root を `EvalSymlinks` で解決してから走査に渡す案はあるが未実装（単体登録経路の `inplace.Register` だけが解決している）
