...
- **前置前に ingest 済みの既存行は移行しない。** 新規 ingest 分だけ `sites/{site}/` が付き、ディスク上は前置あり/なしが混在する。この前提の上に成り立つ断定であり、`sites/` 自体を先頭成分に使っていた既存の `filename_template` があれば話は別（`sites/` は `catalog/` / `thumbnails/` と同じ「新設の予約ディレクトリが過去の運用と衝突しないことを祈る」という一般的なトレードオフを負っている）
- **`rel_path` の `sites/{site}/` 前置をパースする読者がいる。** rescue のストレージスキャン（catalog が 1 世代も無いときのフォールバック）は、前置ありのファイルを前置の site で復元し、前置なしのファイルだけ `--site` にフォールバックする（[rescue.md](rescue.md)）。混在は無害のまま保たれる --- 前置なしの行は前置導入前の単一 site 時代の ingest なので `--site` に単一の site 名を渡す運用で正しく復元でき、前置ありの行は前置自体が site を持つので `--site` の値に関わらず正しく復元できる
