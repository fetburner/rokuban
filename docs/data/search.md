...
1. **EPG は消えるが録画は残る**。`recordings` は放送の事実を自前の列（`title` / `description` / `genres` / `channel_type` / `service_name`）に凍結した永続資産である。詳細は §6「録画した番組の情報は予約〜ingest 時点で録画行に非正規化スナップショット」にある（[projections.md](projections.md)）。`epg_programs` に JOIN して検索すると、EPG のローリングウィンドウから外れた古い録画では条件が**沈黙して 0 件**になる（絞り込みとして最悪の壊れ方）
