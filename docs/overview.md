...
| **外部識別子（API の資源同定という長寿命の契約）** | `programId`。M3-1（issue #29 / #31 / #53）で `(site, programId)` が API のパスとクライアントのクエリキー階層に入り、`docs/schema/principles.md` §1-5 の資源同定そのものになった。爆風半径としては上の「短命・導出」より一段重い —— クライアントのブックマーク・キャッシュキーという、DB のマイグレーションより差し替えコストが高い場所に染み出している |
