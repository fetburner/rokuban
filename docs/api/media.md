...
- **作り直しは旧セッションを完全に止めてから行い、チューナーの解放を待つ。**
  `stop()` は ffmpeg の終了（`<-s.done`）まで待つので、**チューナーを 2 本同時に
  掴む形にはならない**。一方 mirakc は HTTP body の Close と tuner プロセスの解放を
  同期していない（実測 2.35〜4.18 秒）。待たずに投げ直すと、チューナーが埋まって
  いる箱では容量エラーになり、退避経路が**無関係なサービスの idle セッションを
  巻き添えにする**。そのため退避経路と同じ解放待ち（`liveMirakcReleaseWait`）を
  1 回入れる。**代償として切替は待ち時間ぶん遅くなる**（実チューナーでの体感は
  未測定）
