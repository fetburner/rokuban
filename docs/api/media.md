...
- **後任は旧セッションの終了と mirakc の解放を待ってから upstream を取る。**
  ...
  **チューナーを 2 本同時に掴む形にはならない**。一方 mirakc は HTTP body の Close と
  ...
  （実チューナーでの体感は未測定）
- **既知の残差（この変更で消えていない 2 つ）。** 切替が入る直前にセッションを掴んだ
  要求は、掴んだ後でそのセッションを止められると 504 になる。`waitForPlaylist` が
  消えたディレクトリを `playlistStartupTimeout`（既定 15s）ポーリングするためで、
  窓は µs〜ms である（hls.js はプレイリストを取り直して回復する）。
