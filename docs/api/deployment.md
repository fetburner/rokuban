...
| keep-alive アイドル接続の上限 | サーバー自身が `IdleTimeout`（120 秒）でリクエスト間のアイドル接続を切る。前段プロキシの upstream keep-alive タイムアウトをこれより長く取ると、プロキシがまだ生きていると思っている接続をサーバー側が先に切ることになるので、プロキシ側の値は 120 秒以下に揃える |
