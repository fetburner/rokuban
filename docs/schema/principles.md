...
7. **意味を持たない行を作らない**（CLAUDE.md 不変条件 10）
   ...
   - **同じ述語を 2 箇所目のクエリファイルに書く前に view にする。** 述語の一致をコメント（「揃えること」）で守るのは、CHECK で禁止するより弱い —— view なら乖離が表現不可能になる（`program_investments` [#162]、`never_scheduled_events` [#157]）
8. **型の規律**
   ...
   - 違反の実例と解消: `recordings.quality_events`（jsonb）の `recording.never-scheduled` マーカーを同期除外・重なり判定・容量判定の WHERE 軸にしていた（issue #98）のは「内容でクエリするなら型付き列」への違反で、issue #161（`00033`）で `never_scheduled boolean` に昇格して解消した。`quality_events` 自身は内訳ログとして残す（消さない）
...
9. **表は行の寿命で割る**（[CLAUDE.md](../../CLAUDE.md) 不変条件 12。M2 完了後のレビューで言語化）
   ...
   - **この寿命チェックは永続表に対して盲目**（[CLAUDE.md](../../CLAUDE.md) 不変条件 13。M2 完了後の別レビューで言語化。#156）。永続表はどんな列を足しても自明に真が返り網に掛からない。**recordings 本体は「試行の帰結の観測」だけを持つ脊椎で、脊椎（試行の観測 = watcher / reconciler）以外のループが書く状態は `recording_id` を FK に持つ衛星表（`media_assets` がその形）に置く。** 境界: `deleted_at` / `superseded_at` は `recordings_unique_active_event` の部分一意索引の述語が参照するため衛星に出せない。番組スナップショット列群は watcher が一度だけ書くので脊椎に属する。分割の軸は書き手であって概念ではない
