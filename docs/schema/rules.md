...
- **`rule_sites.site` に FK は張らない。** サイトのレジストリは設定ファイルにあり（§1「サイトスコープ」）、外部に真実があるものは存在を制約できない。**存在制約の代わりに、書き込み時の照合を置く**（`validateRuleSites`、`internal/api/rules.go`）。各 site 名をそのプロセスが読んでいる `config.mirakcs` レジストリ（`GET /api/sites` と同じ一覧）に照合し、未知（タイポ含む）なら 400 を返す。空文字列も同じく 400 で、「未指定 = 全サイト」を意味する個別要素としては扱わない（絞り込みの意図が黙って全サイトへ反転するのを防ぐ。全サイトを意図するなら `sites` 全体を省略か空配列にする）
- **照合を「保存済みの名前」に及ばせない。** 更新では、そのルールに既に保存されている site 名はレジストリに無くても通す。子表は全置換なので、更新では `sites` を毎回載せ直す必要がある。クライアントは GET で得た値をそのまま送る（web の編集フォームもそうしている --- `web/src/lib/program-search.test.ts` の preserve テスト）。保存済みの名前まで照合すると **site をリネーム・削除した瞬間、その site を持つ全ルールが名前や優先度を直すだけの編集も含めて保存不能になる** —— レジストリから消えた site の掃除は書き込み時照合の仕事ではない。照合が狙うタイポは定義上「保存済みに無い新しい名前」なので、免除しても検出力は落ちない（`TestRuleSites_RegistryDriftAllowsRoundTripUpdate` が両方向を押さえている）
- **免除の境界はルールの同一性であり、作成には及ばない（未解決）**。免除集合はそのルールの `rule_sites` なので、**既存ルールを下敷きに別のルールを作る経路には効かない**（検索画面のフォーク。[フロントエンド](../frontend/search.md) §「UI が持たない次元は勝手に埋めない」）。レジストリから site が消えた後も、その site を持つルールは編集（`PATCH`）が通る。フォーク（`POST`）だけが 400 `unknown site` になる（`TestRuleSites_RegistryDriftRejectsFork`）。`sites` は条件 UI に出していないので UI 内で外すこともできず、復旧手段は設定を戻すか API を直叩きするかの 2 つだけ。免除が守りたい境界は本当は「クライアントが GET で読んで載せ直した名前」であって「そのルール」ではない。**未解決: 免除を作成側にも届かせる（複製元を受け取る）か、フォーク時に web が「レジストリに無い site」を見せて明示的に外させるか**。どちらも API / UI の形を決める判断なので書き込み時照合と同じ PR では決めない（黙って落とす選択だけは取らない —— 絞り込みが無音で全サイトへ反転する）
- **空文字列の拒否を `items.minLength: 1` にしない**。このリポジトリには OpenAPI のリクエスト検証ミドルウェアが無い（`kin-openapi` 系の依存は無し、`minLength` は `openapi.yaml` 全体で 0 件）。スキーマに書いた制約を強制する主体が居ない。強制できるのは `validateRuleSites` だけであり、保存済み名の免除もスキーマでは表現できない。そこで同じフィールドの検証権威を 2 つに割らず、ハンドラ 1 箇所に集める（`openapi.yaml` の description は 400 になる条件の記述に留める）
