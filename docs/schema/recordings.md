...
- `program_snapshots` への FK は張らない。snapshot は放送 + 猶予で GC されるが、欠測は EPG の放送地平を超えて残す観測である。この表の読者はいずれも `reservations` 行を経由して届き、`reservations` は snapshot と一緒に放送 + 猶予で先に GC されるので、欠測行を `retention_grace + 30日` まで残しても読者からは観測されない。そのため `ruler` が `retention_grace + 30日` で刈る。snapshot の CASCADE と同時に消すと GC 後に同期除外が外れ、終了済み予約が再び schedule 対象になる
