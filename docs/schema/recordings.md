...
- `program_snapshots` への FK は張らない。snapshot は放送 + 猶予で GC されるが、欠測は EPG の放送地平を超えて残す観測である。通常はこの表の読者が `reservations` 行を経由し、`reservations` は snapshot と一緒に放送 + 猶予で先に GC されるので、欠測行を `retention_grace + 30日` まで残しても読者からは観測されない。そのため `ruler` が `retention_grace + 30日` で刈る。ただし `program_id` 再利用で同じ放送イベントキーから新番組が到達可能になる経路では、この前提は成り立たない（[reservations.md](reservations.md) §3.7 の未解決の窓）。snapshot の CASCADE と同時に消すと GC 後に同期除外が外れ、終了済み予約が再び schedule 対象になる
...
`need-rescheduling` 等で `status='failed'` の行がこの枠を占有したまま残っているところに、mirakc が同一 active-event を後で録り直して成功 record を報告することがある（delayed broadcast、mirakc 側の手動再録画等）。この成功 record は無条件で枠を得られなければならない。欠測（`never_scheduled_events`）は別表なので supersede の対象ではない —— 寿命内に本物の record が来ても欠測行は残り、`recordings` には試行行だけが増える。
