...
| `rokuban_live_session_evictions_total{reason,result}` | Counter | 起動失敗からの再試行のために退避したライブセッション数（`reason`: `upstream` / `session_limit`、`result`: `retry_succeeded` / `retry_failed`） |
