...
- **site を持たないブレーカー（`delete_reconcile`。`internal/breaker.IsSiteless`）だけは `POST /api/breakers/{name}/resume` で再開する。** site 列が実サイト名を持たないため、site をパスに要求すると `config.mirakcs` に `default` という名前の site が無い構成では一切解除できなかった（[docs/api/rest.md](../api/rest.md)「エンドポイント設計の規約」）
