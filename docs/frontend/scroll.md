...
下端が揃うようクランプされるためである。しかし番組リストは 1 時間窓ぶんの番組が
初回表示（スクロール前）で既にモバイルのビューポートより長いことがあり、その場合
たまたま行の境界がタブの上端とずれた位置に来ると、時刻や「予約」ボタンを含む行が
タブの裏に半分だけ隠れた状態でユーザーの目に入る。

これは `padding-bottom` の設定ミスではなく、スクロール前の状態そのものに
構造的に空いている穴なので、**初回表示（`window.scrollY === 0`）に限定して**
補正のスクロールを 1 度だけ行う（`components/program-list.tsx`）。判定・補正量の
計算は純関数（`lib/tab-clearance.ts` の `computeInitialTabClearanceScroll`）に
切り出してあり、DOM 読み取りと `window.scrollBy` の実行だけが副作用側の役割
（`captureAnchor` と同じ切り分け。上記「テストの範囲」参照）。

タブの上端は `main` の `padding-bottom` から逆算せず、`BottomTabs` の
`<nav data-testid="bottom-nav">` を直接測る --- `nav` 自身の `border-t`（1px）
ぶん、実際の描画上の上端が `padding-bottom` からの逆算より高い位置に来ることを
実機で確認したため（逆算のままだと補正してもなお 1px 重なりが残る）。

日付ジャンプ後の再チェックはこのスコープの外 --- 再現手順が「開いてスクロール
せずに見る」だったため初回表示に限定した。合否は `web/e2e/programs-bottom-nav.mjs`
（jsdom は `getBoundingClientRect` を計算しないため原理的に検出できない）。
