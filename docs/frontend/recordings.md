...
**進捗は REST の再取得で収束させる**（SSE はヒント。不変条件 5）。取り込み中
（または録画中）の録画が読み込み済みのページにある間だけ 5 秒間隔で再取得し、
無くなれば止める（`lib/ingest.ts` の `ingestRefetchIntervalMs` / `isIngestInFlight`）。
一覧は無限リストなので、常時ポーリングすると積んだページ全部を取り直すことになる。
