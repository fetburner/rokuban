...
- **ルールは絞り込みパネルの「ルール」節から選べる。** 候補はルール一覧の返す順で
  ...
  同じ一覧からルール名を解決し、未解決の間は `ルール #N` と表示する。
  `ruleId` を選ぶと `source` は解除し、0 件になる「手動」「帰属なし」は選べないように
  する。どのルールかが分かれば出自をさらに問う必要がなく、表現できない組み合わせを
  残すより、条件を設定したのに結果が 0 件になる理由を探させない方が明確だからである。
