...
- **ルール名の解決は `useListRules` のキャッシュから引く。** ルール専用の単体
  ...
  --- `RulesPage` が常に全件を引く設計なので一覧はどの画面でも温まっている
  前提に乗れ、録画の展開ごとに個別の 1 件取得を増やす理由がない。**キャッシュ
  が空の状態で複数の展開行が同一コミットでマウントされた場合**は取得が 1 回
  にまとまることを `recordings.test.tsx`「ruleId を持つ複数の行を同一コミッ
  トで展開すると /api/rules の取得は 1 回にまとまる（react-query の初回
  フェッチの重複排除）」で固定した。片方が解決してキャッシュに乗った後で
  別の行を展開する（実利用でより典型的な順序）場合は、既定の
  `staleTime: 0` によりマウントごとに refetch が起きる（同テストで実測）ので
  「常に 1 回にまとまる」わけではない
- **`rules.find` で見つからない ruleId は `#N` 表記に落とす。ただしこれは
  ...
  落ちない。`#N` に落ちるのは、一覧クエリが未解決（初回展開時は `#N` から
  ルール名へ表示が切り替わる）・取得失敗・キャッシュが古くて該当 id がまだ
  乗っていない、という一時的な状態
