...
- **ルール名の解決は `useListRules` のキャッシュから引く。** ルール専用の単体
  ...
  `renderPage`（未指定 = 0）で設定が違うため、同じ操作でも一致しない（同一
  シナリオの実測で `/api/rules` は未指定 = 2 回 / 30_000 = 1 回）。**設定依存の
  量を、設定が本番と違うハーネスで測って一般の挙動として書かない** --- docs /
  コメントに残す主張は設定に依存しない性質（「行ごとの取得を発行しない」）に寄せる
