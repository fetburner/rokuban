...
- **シークプレビュー**: 動画の下のスクラブ帯でポインタを動かすと、その位置に対応する
...
- **プレビューはネイティブ controls のシークバーに重ねない。** ネイティブのシークバーは
  ...
  ホバーとクリックが同じ式から出る。帯はマウス専用の補助として `aria-hidden` にし、
...
- **タイルが無い録画ではプレビューが出ず、poster だけの従来の見た目に戻る。**
  未生成・生成失敗・タイル導入前に原本を消した録画がこれに当たる。 配信側は未生成なら 404 を返し、クライアントはそれを
  「プレビューを出さない」として扱う（再生面は壊れない）。ホバーの位置は実ブラウザで
