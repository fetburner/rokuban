...
は呼ばない。** `<video preload="metadata">` はメタデータだけを先読みするが、実際の
データ転送（本編の取得）はブラウザが `.play()` 相当の処理を始めたときに重くなる。
行のボタンをワンタップしただけで本編の転送が始まると、一覧をスクロール中の誤タップが
そのまま通信量になり、コストのかかる操作を暗黙に始めないという値札の方針（M7）と
衝突する。ネイティブ `<video controls>` の再生ボタンをもう一段挟むことで、実際の
データ転送は利用者の最後の明示的なクリックに紐付く。予約のワンタップ + トーストとは
非対称だが、予約は DB 行を作るだけの安価な操作、再生は帯域を伴う操作なので意図的に
揃えていない。
