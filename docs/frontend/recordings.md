...
がストレージを最も直接に消費する画面であるため最初の設置先にした。ホームへの
移設を前提に `components/` に独立させてある。
...
冒頭に偏ると実際より早い/遅い満杯見込み日を出し、後者は下界主義に反する過小
警告の方向に振れるため（`lib/storage-forecast.test.ts` の「予約が窓の終盤/冒頭
1 件に集中しているとき」の 2 テストがこの 2 方向を固定している）。
