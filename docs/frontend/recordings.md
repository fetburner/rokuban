...
見込みと現在の観測時刻は「詳細」を展開した側へ移す。これにより、視聴対象へ
向かう録画行の位置を下げずに、必要な容量情報へは 1 操作で到達できる。
`<details>` を展開すると、`media`（アーカイブ）と、設定されている場合の
`scratch`（ローカルスクラッチ）を名前・総容量・使用済み・空きに分けて表示する。
単一世帯では録画一覧から容量を確認できれば足りるため、専用ページは作らない。
見る頻度やストレージ root の種類が増えた時点で専用ページを検討する。

ただし、情報量を減らすために警告を隠してはいけない。満杯見込み日、1 時間を
超えた観測の「古い可能性」、およびストレージ観測そのものが無い／取得できない
場合の沈黙は従来どおり扱う。警告がある場合は該当する見込み・観測情報を要約行へ
戻し、エンコードの「待機中 N 件 / 実行中 N 件」も 0 件を含めて省略しない。

### issue #686 の配置再評価

視聴対象へ到達する距離を、API モックを使った実 Chromium の録画一覧で、固定ヘッダー
とモバイル下部ナビを考慮して計測した。値は 360 / 390 px 幅とデスクトップ幅で、
最初の録画詳細リンクの viewport 上端（および必要スクロール量）を取ったもの。
これは利用者の操作ログではなく、配置仮説を比較するためのレイアウト計測である。

| 状態 | 360 px | 390 px | デスクトップ |
|---|---:|---:|---:|
| 通常（変更前 → 変更後） | 317 → 260 px | 317 → 260 px | 245 → 208 px |
| 容量警告（変更後） | 326 px | 306 px | 208 px |
| 観測 stale（変更後） | 342 px | 342 px | 208 px |
| 警告多数（変更後） | 511 px | 495 px | 305 px |

通常状態では管理情報行を 43 px に収め、見込み・観測時刻を詳細側へ移した。容量・
stale・多数警告・エンコード待機列・ストレージ取得失敗・観測なしの各フィクスチャを
実ブラウザで確認し、警告文と件数が要約から消えないこと、ストレージ欠損時に
「+0 B」などを捏造しないことを固定した。警告リスト自体は件数で切らない。
