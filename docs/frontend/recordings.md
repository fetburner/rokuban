...
- 再生速度は `ratechange` で `rokuban:playback-rate` に保存する。端末ごとに 1 つの値を
  VOD と追っかけ再生で共有し、それぞれの `<video>` の `defaultPlaybackRate` に適用する。
...
- **シークプレビュー**: 動画の下のスクラブ帯でマウスを動かすと、その位置に対応する
  ...
  純関数が「再生位置 → 格子位置」を出す
- **プレビューはネイティブ controls のシークバーに重ねない。** ネイティブのシークバーは
  位置も幅もブラウザごとに違い、外から測れない。重ねると「見えたタイル」と「クリックで
  飛ぶ先」がずれる（3 時間の録画なら 16px で約 4 分）。映像全体のホバーに出すと、
  ポインタを置いたまま視聴する間プレビューが映像を覆い続ける。帯は座標を自分で持つので、
  ホバーとクリックが同じ式から出る。帯はポインタ操作の補助として `aria-hidden` にし、
  キーボードの経路はネイティブ controls が持つ
- **プレビューはマウスだけに出す。** タッチでは帯のタップがシークだけに効く。タップには
  離脱のイベントが来ないので、出すとプレビューが映像を覆ったまま残る
- **帯が 1 枚ぶん（320px）より狭い画面では、プレビューを帯の幅まで縮めて出す**
- **追っかけ再生のタイムラインとは別の部品である** —— あちらは「録画済み / 未録画 /
  予定終端」を描く帯で、こちらは再生済み割合とプレビューだけを持つ
- **タイルは最初にポインタが触れたときだけ取りに行く。** マウント時に先読みすると、
  3 時間の録画で 2 MB 程度を、一度もホバーしない利用者にも払わせることになる
- **タイルが無い録画ではプレビューが出ない**（帯はシークと再生済み割合のために出る）。
  未生成・生成失敗・タイル導入前に原本を消した録画がこれに当たる。配信側は未生成なら
  404 を返し、クライアントはそれを「プレビューを出さない」として扱う（再生面は壊れない）。ホバーの位置は実ブラウザで
  しか測れない（jsdom は `getBoundingClientRect()` が常に 0）ので、判定は
  `web/e2e/seek-tiles.mjs` にある
...
上記のキー操作は自前 UI のボタンではなくページ単位の追加ショートカットである。標準
controls と同じキーイベントを二重処理しないよう、`<video>` や他の操作要素にフォーカスが
あるときは無視する。
...
- **行のバッジ**（`IngestBadge`。一覧の行と単体ページのヘッダー）: 「取り込み待ち」/
  ...
  いなければ「（停滞）」を添える
  - **録画中は割合を出さない。** 追従 ingest は録画開始から走るので、録画中の
    分母は「mirakc がその時点で観測しているサイズ」であり最終サイズではない。
    `writtenBytes` がそれを追い越すことがあり、割合にすると「録画全体の 100% を
    取り込んだ」と読める嘘になる（実機で観測した）。録画中はバイト数だけを出し、
    % は分母が確定する録画終了後から出す
