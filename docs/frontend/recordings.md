...
`sizeBytes` の省略だけを見ていた頃は未 ingest と区別が付かず、権限不足で ingest が
リトライ中の録画に「削除済み」と読める表示が出ていた（[経緯と失敗事例](#経緯と失敗事例)）。
...
- **`pending`**: いつ始まるかはこちらから分からない。**失敗し続けている ingest も
  ...
  落ちるので、River が再試行し続ける間ずっと `pending` のまま（#211 で実際に
  起きた壊れ方）
...
対象にしてはならない** --- `record_sync` 行が消えない以上「未完了」は恒久的に
残りうる（下記「経緯と失敗事例」）。
...
- ブラウザ再生は M3-5、ごみ箱ビューの非表示は M3-18、録画検索の同居は M3-25 で
  実装した
- **未 ingest の録画に「原本が削除済み」と読める表示を出していた**（issue #211 /
  issue #212）。`sizeBytes` の省略は「取り込んだ後に消した」と「まだ取り込めて
  いない」の両方を含むのに、UI はそれを「削除済み」と断定していた。実環境では
  `/mnt/media` の書き込み権限不足で ingest が `permission denied` のままリトライ
  していただけで、権限を直せば後から取り込める状態だった。区別に必要な材料
  （原本 `media_assets` 行が `state` を問わず存在するか）が API に無かったことが
  原因で、`Recording.ingest` を足して解消した。**併せて、進捗表示が無かったこと
  自体が調査を遅らせた** --- 「止まっているのか進んでいるのか」が画面から読めず、
  ログにも完了時の `ingest: transfer complete` しか出ていなかった
- **その `Recording.ingest` の最初の実装が、同じ誤りを `pending` について作り直した**
  （PR #323 のレビューで指摘・実測）。`pending` の根拠を `record_sync` 行の**存在**に
  取っていたため、ingest ジョブが一度も投入されない `failed` / `canceled` の録画
  （watcher が投入するのは mirakc の record が `finished` のときだけ。`record_sync`
  行は消えない）が**永久に「取り込み待ち」を名乗った**。同時に、未完了状態すべてを
  5 秒ポーリングの対象にしていたため、**失敗録画が 1 件でも読み込み済みページに
  あればポーリングが恒久化**し、docs に書いた「無くなれば止める」が成立していなかった。
  直し方は 2 つで、(1) `pending` の根拠を「watcher が ingest を投入する条件と同じ述語」
  （`record_sync.status = 'finished'`）に揃える、(2) 短い周期を張る条件を「未完了」
  ではなく**「進捗の数字がいま動いている」**に狭める。**教訓は「状態の名前が
  『これから起きる』を含むなら、起きる根拠を述語として書けるか確かめる」**
