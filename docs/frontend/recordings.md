...
- **直近の録画実績が 0 件**（平均ビットレートの標本が無い）のときは残高だけ出し、
  「+Y GB の見込み」「満杯見込み」は出さない --- でっち上げの既定値を置かない
- **見込み消費が残量に収まる**ときは満杯見込み日を出さない（下界主義。
  `lib/capacity.ts` の「主張は下界に限る」と同じ精神。見込みは 2 つの近似
  --- 直近標本の外挿、7 日間への均等分布の線形外挿 --- の上に立っており、
  「収まる」ことを保証できるほど正確ではないため）

録画・予約の取得自体が失敗した場合も見込みの算出には進まず、2 番目と同じ「残高だけ
出す」に縮退させる。ヘッダーの補助情報 1 つのために専用のエラー表示を割く対象では
ないため。
...
UI 側で必ず判定する。worker の既定観測間隔は 5 分だが設定で変更可能で API には
表れないため、「既定間隔の何倍か」ではなく「観測ループが実際に止まっていることを
検知するための独立した余裕」として 1 時間（`lib/storage-forecast.ts` の
`observationStaleAfterMs`）を選んだ --- 既定間隔に対して 12 倍の余裕があり、1 回の
失敗パスや再起動直後の遅延では誤って「古い」と出ない一方、観測ループが本当に
止まっていれば 1 時間以内に検知できる。設定で間隔を大きく延ばした運用では基準が
弱くなるが、しきい値を設定可能にする要求が実際に出るまでは固定値のままにする。
...
満杯見込み日は、見込み消費を 7 日間に均等に分布すると仮定した線形外挿。実際の予約は
曜日・時間帯に偏るが、日ごとの偏りを再現する根拠が無いための単純化 --- 桁を外す
近似ではないが、この単純化のために「満杯見込み日」は目安であって確約ではない。
