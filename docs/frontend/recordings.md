...
- **シークプレビュー**: 再生面の上でポインタを動かすと、その位置に対応するタイル画像
  （`/api/media/recordings/{id}/seek-tiles`）を小さく重ねて出す。タイルはサーバー側の
  固定値（10 秒間隔・10 列）で、`lib/seek-tiles.ts` の純関数が「再生位置 → 格子位置」を
  出す。**追っかけ再生のタイムラインとは別の部品である** —— あちらは「録画済み /
  未録画 / 予定終端」を描く帯で、こちらは動画の上に浮く 1 枚の画像である
- **タイルは最初にポインタが触れたときだけ取りに行く。** マウント時に先読みすると、
  3 時間の録画で 2 MB 程度を、一度もホバーしない利用者にも払わせることになる
- **タイルが無い録画（未生成・古い録画・生成失敗）ではプレビューが出ず、poster だけの
  従来の見た目に戻る。** 配信側は未生成なら 404 を返し、クライアントはそれを
