...
- **追っかけ再生には画質セレクタを出さない（この判断は未解決ではなく決定である）。**
  `LivePlayer mode="chase"` は `profile` prop を受けて `?profile=` を組み立てられる
  ので、配線そのものは既にある。出していないのは選択を置く場所が無いためである
  --- 追っかけの画面は `/recordings/{id}#chase`（録画詳細）にあり、その search の
  `profile` は**再生位置のキーに使う VOD 側のプロファイル名**として既に使われて
  いる。同じキーを live のプロファイルにも使うと、1 つの URL パラメータが 2 つの
  設定を指すことになる。追っかけにも画質を出すなら、別のキーを足す判断が要る
