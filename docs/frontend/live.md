...
- **`?audio=` を載せるのは probe の 1 回だけである。** `LivePlayer` は probe の URL
  ...
  いると後から選んだ方が勝ち、先に選んだ側の音声は巻き戻る**（その視聴者の
  ストリームは 1 回切り替わる）。これはストリーム側が切替を直列化している
  （最後の要求が残る）ことから決まる。**体感（切替のたびに何秒止まるか）は
  未測定**である。非対称（probe には載る / プレイヤーには載らない）は
  `live-player.test.tsx` が両側を固定する
