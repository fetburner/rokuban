...
**音声（二重音声の主/副）を選ぶ UI を持つ**（issue #870）。`標準` / `主音声` / `副音声`
の 3 状態を UI に出し、明示した主/副だけを `?audio=` に持つ。`標準` は
`?audio=` を付けない（= ffmpeg の既定引数。二重音声では主音声が左・副音声が右の
ステレオとして出る）。**一度主/副を選んだ後に標準へ戻す操作は、共有セッションの
他の視聴者を巻き込むため適用しない**。戻すときは再生を停止してからページを再読み込みする。
...
- **`?audio=` を載せるのは probe の 1 回だけである。** `LivePlayer` は probe の URL
  ...
  作り直しを要求することになり、同じチャンネルを別の音声で見ている 2 人が互いの
  音声を作り直し合って両方止まる。**この非対称（probe には載る / プレイヤーには
  載らない）は実装の見落としに見えるので、`live-player.test.tsx` が両側を固定する**
- **切替は probe のやり直しになる**（セッションが作り直されるので、古いセグメントは
  もう無い）。画質の切替と違い**再生は数秒途切れる**。ただし `LivePlayer` は
