...
**停滞したら画質を 1 段下げる**（issue #871）。回線が細い場所やエンコードが
追いつかないときに、永久にリバッファするかエラーを出すかしかない状態を減らす。
...
- **止まった理由を「回線」と「配信側」に分けない。** 取る行動が同じで、分けるには
  帯域の推定が要る（測っていない数字を出さない）
