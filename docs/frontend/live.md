...
**音声（二重音声の主/副）を選ぶ UI を持つ**。`標準` / `主音声` / `副音声` の 3 状態を
UI に出し、明示した主/副だけを `?audio=` に持つ。`標準` は `?audio=` を付けない
（= ffmpeg の既定引数）。**二重音声の放送で各状態が実際に何を出すかは未検証**である
（この環境に実チューナーが無く、二重音声の TS を用意できない）。一度主/副を選んだ後は
標準を disabled にする。既定へ戻すにはセッションの作り直しが要り、同じセッションを
見ている他の視聴者の音声を巻き戻すためである。戻すときは再生を停止してからページを
再読み込みする。

- **画質と違って、選択肢は常に 2 択で出す。** どの番組が副音声を持つかを Rokuban 側で
  知る手段が無い。音声 ES の情報を持たず、二重音声は 1 本の AAC ES の L/R なので
  ffprobe では通常のステレオと区別できない（区別できるのは記述子だけで、それは
  不変条件 6 の境界。[api.md](../api.md) §実装）。「一覧が 1 件なら出さない」という
  画質の規律はここでは適用できない（選択肢の数が放送に依存しないため）
- **副音声の無い番組で副音声を選んでも何も起きない。** `-dual_mono_mode` は通常の
  ステレオでは無効である（ffmpeg 9.0.2 で実測。4 通りの出力がバイト一致）。
  だから「副音声がありません」の表示も、選択を無効化する判定も置かない
- **選ぶだけでは probe もセッションも起こさない**（画質と同じ。「選ぶ」と「流す」の
  分離は同節の `playingKey` / `selectedKey` が担う）
- **`?audio=` を載せるのは probe の 1 回だけである。** `LivePlayer` は probe の URL
  にだけ音声を載せ、hls.js / ネイティブ `<video>` に渡す URL には載せない。
  `-dual_mono_mode` は ffmpeg の入力側オプションなので 1 回の起動で両方の音声は
  出せず、**要求された音声が今のセッションと違えば streamer はセッションを作り直す**
  （[api.md](../api.md) §実装）。載せ続けると数秒ごとのプレイリスト再取得が毎回
  作り直しを要求することになる。**同じチャンネルを別の音声で見ている 2 人が
  いると、最後に置き換えた方の音声が残り、もう片方の音声は巻き戻る**（その
  視聴者のストリームは 1 回切り替わる）。ストリーム側が切替を直列化している
  ためで、**同時に要求された場合はどちらが勝つかは直列化の順に依存する**（到着順
  ではない）。**体感（切替のたびに何秒止まるか）は未測定**である。非対称
  （probe には載る / プレイヤーには載らない）は `live-player.test.tsx` が
  両側を固定する
- **切替は probe のやり直しになる**（セッションが作り直されるので、古いセグメントは
  もう無い）。`LivePlayer` は unmount しない（`key` も同意判定も触らない）ので、
  再生の同意を取り直す操作にはならない。**切替の間、同じチャンネルを見ている他の
  視聴者のプレイリスト要求は後任の準備完了まで待たされる**（解放待ち + 起動。
  [api.md](../api.md) §実装）--- 空の窓に別セッションを作らせない代償である
- **離脱ヒントは送らない。** 音声の切替は「このチャンネルを見るのをやめた」では
  ない（離脱 effect を `audio` に依存させない）
- **チャンネルを切り替えても音声を保つ**（チャンネル一覧のリンクが `?audio=` を運ぶ）
- **値域は閉じているので `validateSearch` で全部検査できる。** 未知の値は
  `undefined` を明示代入して落とす（streamer は 400 `unknown live audio` を返す）
