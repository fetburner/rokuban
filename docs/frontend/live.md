...
追っかけの URL は次の固定深さで、site や mirakc の record id をブラウザ側に持たない。

```
/api/recordings/{recordings.id}/chase/playlist.m3u8[?profile=<name>]
/api/recordings/{recordings.id}/chase/segments/{name}
/api/recordings/{recordings.id}/chase/leave
```
...
`rokuban:playback:{recordingId}:{profile}` を共有する。録画 ID とプロファイルが同じなら、
追っかけと完了後の VOD で「続きから」が一致する。先頭付近と終端 5 秒以内は保存しない。
