...
hls.js には `startPosition: 0` を渡す。録画詳細には番組開始から終了予定までの
タイムラインを置く。予定範囲は `startAt` から `startAt + durationMs` まで、録画済みの
塗りは `now - startedAt` から導出して 1 秒ごとに伸ばす。つまみは `availableChaseSeconds`
以内に制限し、サーバーが追いつくための 1 秒の余裕を残す。ドラッグ中は位置だけを
プレビューし、離したとき（キーボードでは操作キーを離したとき）に
`/chase/offset/{offset}/...` へ切り替える。表示は番組開始からの実時刻と経過を使い、
秒数入力と「この位置から再生」ボタンは置かない。

`durationMs` は録画作成時の番組スナップショットで、watcher は追っかけ中の延長に合わせて
...
**プロファイル（画質）を選ぶ UI を持つ**（issue #869）。選択肢は
`GET /api/live-profiles`（`config.live.profiles` の `name` と表示用の `height`）から
作り、**並びは設定順のまま**にする。先頭が `?profile=` を省略したときの既定なので、
サーバー側の決め方がそのまま既定の根拠になる。表示名は `height` を添える
（`hd（720p）`。0 は「スケールしない」なので名前だけ）。

- **選択肢が 1 件以下ならセレクタを出さない。** 選ぶ余地が無いコントロールは
  「機能しないコントロール」である（issue #209 と同じ規律）。一覧が 0 件になるのは
  `live.profiles` が未定義のときだけである --- **`live.enabled: false` でも
  profiles が書かれていれば一覧は返る**（`GET /api/live-profiles` は config の写し。
  [api.md](../api.md) §一覧の契約）。その構成では画面自体が「無効です」の分岐に
  落ちるので、セレクタは出ない
- **切替はセッションを作り直さない。** 1 サービス = 1 ffmpeg が全プロファイルを
  同時に出力しているので、替わるのは同じセッションのプレイリストの URL だけである
  （[api.md](../api.md) §資源同定）。`LivePlayer` を unmount せず、`key` も
  同意判定（`playingKey` / `selectedKey`）も触らない --- **画質の切替を「同意を
  取り直す」操作と誤読させる形にしない**
- **離脱ヒントも送らない。** ヒントは「このチャンネルを見るのをやめた」の合図で、
  画質の切替ではない（`LivePlayer` の離脱 effect を `profile` に依存させない）
- **選択は `?profile=` に持つ。** `routes.tsx` の `validateSearch` は**形だけ**を
  見る（非空の文字列か）。落とすときは `undefined` を明示代入する。
  **実在の判定はそこでは書けない** --- 選べる名前は実行時に行を引く
  `GET /api/live-profiles` から来る。照合は一覧を読める `pages/live.tsx` が
  `validLiveProfile`（`lib/live.ts`）で行う。`?site=` と同じ分担である
- **未知の名前は落ちて既定（先頭）に倒す。** streamer は空の `?profile=` を既定に
  落とすが、**未知の名前は 400**（`unknown live profile`）を返す。落とさないと、
  綴り違いの共有リンク・古いブックマークがエラー画面になる
- **既定は URL に書き戻さない。** 明示的に選んだ値だけを載せる（既定は両側で
  同じ先頭プロファイルに解決するので、URL に書く意味が無い）
- **チャンネルを切り替えても画質を保つ**（チャンネル一覧のリンクが `?profile=` を
  運ぶ）。画質は「この局」ではなく「この端末の回線」の性質である
- **追っかけ再生にも画質セレクタを出す。** `LivePlayer mode="chase"` は `profile`
  prop を受けて `?profile=` を組み立てられるので、streamer 側の配線は既にある。
  選択は `/recordings/$id` の search に `?liveProfile=` として持つ。追っかけの画面は
  `/recordings/{id}#chase`（録画詳細）にあるが、`#chase` 側に持たせるとハッシュを
  変えるたびに録画詳細のコンポーネントを作り直し、再生位置の復元もやり直すためで
  ある。search はその再マウント用 key に含めないので、画質の切替で `LivePlayer` は
  unmount しない。

  キー名を `profile` にしないのは、この画面に `encode.profiles`（完了後の VOD）と
  `live.profiles`（追っかけ）の 2 軸が同居し、素の `profile` ではどちらを読むのか
  決まらないためである。`liveProfile` は設定キーと一覧 API（`GET /api/live-profiles`）
  の名前に一致する。URL に明示された値は一覧で検証し、未知の値は既定（先頭）に落とす。
  一覧が 2 件以上のときだけセレクタを表示し、0 件・1 件では既定のプロファイルで
  再生する。
  ...
  保持ディレクトリに全プロファイルのプレイリストが残るためである。GC 後は現行の追っかけと
  同じ 404 になるので、UI に特別な分岐は置かない。根拠は偽 ffmpeg による
  `TestFinishedChaseProfileSwitchServesRetainedPlaylists` で、実 ffmpeg では未測定である。

**停滞したら画質を 1 段下げる**（issue #871）。回線が細い場所で、永久に
リバッファするかエラーを出すかしかない状態を減らす。**効くのは回線（配信の転送）が
律速のときだけである。** 1 サービスの ffmpeg 1 本が全プロファイルを同時に出力して
いるので、クライアントが下げてもエンコードの負荷は変わらない。エンコードが
追いつかずに止まる場合は、12 秒ごとに最下段まで下がり続けて効果が無い
（止まった理由を分けない判断の代償。下の「理由を分けない」を参照）。
判定の信号は経路で違うが、**閾値は `liveStallTimeoutMs`（12 秒）1 つ**である。

- **ネイティブ経路（Safari）**: `stalled` / `waiting` の既存の猶予が満了したら、
  従来のエラー表示の代わりに**まず降格を試す**。下げ先が無ければ現行どおりの
  エラー文言に落ちる（**新しい失敗経路・新しい文言を作らない**）
- **hls.js 経路**: 計器の 1 秒ポーリング（issue #476）に相乗りして
  **`currentTime` が 12 秒進まない**ことを見る。`stalled` / `waiting` は MSE では
  正常時にも何度も出るので聴けない。**下げられないときは何もしない** ---
  現行の hls.js 経路に停滞の失敗経路は無いので、ここで新設すると
  **プロファイルが 1 件しかないデプロイの挙動まで変わる**
- **`paused` の間と非表示タブでは数えない。** `<video>` に `autoPlay` は無いので、
  再生を押す前は `currentTime` が進まないのが正常である。非表示タブでは
  `setInterval` が間引かれるので、基準を捨てないと復帰時に誤発火する
- **自動再開を待っている間は「利用者が一時停止した」抑止を外す。** 切替の cleanup が
  `load()` で `paused` にした状態を「利用者が止めた」と読むと、**降格先の配信も
  死んでいて `canplay` が来ない場合に停滞の検出ごと止まり、黒いまま永久に何も
  出ない**（降格もエラー表示も起きない）。`resumePending` の間だけ抑止を外す
  （`canplay` が来たら戻す）
- **「一度でも再生が始まったか」は effect を跨いで持つ。** 切替は effect を
  張り直すので、effect ごとに持つと**利用者が自分で一時停止した video を
...
- **止まった理由を「回線」と「配信側」に分けない。** 分けるには帯域の推定が要る
  （測っていない数字を出さない）。**代償として、配信側（エンコード）が律速の停滞でも
  下げる。** 下げても負荷は変わらないので最下段まで進み、その画質がチャンネルを
  跨いで残る。利用者がセレクタで選び直せば戻る

**下げ先の選び方は設定順で前方へ走査し、実効高さが現在より小さい最初の段である**
（`nextLowerProfile`）。実効高さは `height > 0 ? height : Infinity`（0 / 省略 =
スケールしない = 元の解像度 = 最重）。`[hd 720, sd 480]` なら sd、`[sd 480, hd 720]`
なら**下げない**。**「配列は後ろほど軽い」という新しい契約は置かない** --- 置くと
`[sd, hd]` の構成で停滞時に重い方へ上げてしまう。順序は運用者の宣言のまま使い、
`height` は「上がってしまう段」への拒否権としてだけ使う（帯域の完全な代理ではないが、
拒否権なら誤りが「下げない」側に倒れる）。

- **片方向である。上げ直しはしない。** 上げるには「もう足りている」の判定 = 帯域の
  能力推定が要り、フラッピング抑止用に**測れない 2 つ目の数値**も要る。上げたい
  ときは利用者がセレクタで選ぶ
- **クールダウンは置かない。** 下げると `LivePlayer` の effect が張り直されて計測が
  0 に戻るので、次の発火には再び 12 秒の無進捗が要る（観測窓そのものがクールダウン）
- **明示選択は上書きしない。** `?profile=` の有無がそのまま「自動が効くか」である。
...
- **追っかけ再生（`mode="chase"`）では自動降格しない。** 下げ先（`?liveProfile=`）は
  追っかけの画面にもある。それでも下げないのは、この降格が `pages/live.tsx` の
  `autoProfile` に配線されており、同じ下げ方を追っかけのセレクタへ広げるのが別の
  判断だからである。**再生再開も
  ライブだけ**にする --- 追っかけは位置指定
  （`startOffsetSeconds`）や保存位置の復元が `canplay` と競合しうるため、今回の
  自動降格の再開経路を広げない
- **降格のあとは再生を再開する**（手動の画質選択でも同じ。持ち越しは cleanup の
  時点の `paused` だけを読み、切替の理由を区別しない）。エラー後の再読み込みで
  再開するのは**ネイティブ経路だけ**である。ネイティブの停滞エラーは `waiting` の
  まま `paused=false` である。hls.js は fatal エラーで `destroy()` が `load()` を呼び
  `paused=true` にするので、再読み込み後は利用者が再生を押す。切替の cleanup は
  ...
  だったときだけ呼ぶ（初回のマウントで呼ぶと「再生」ボタンによる同意の分離が壊れる）。
  **これが無いと、降格は画質を下げるだけで利用者は黒い画面のまま**になる
  （実測: セグメントを復旧させても `currentTime` が止まり `paused=true` のまま）
- **下げたことは情報欄に残す**（`映像が止まったため、画質を sd（480p）に下げました`。
  プロファイル名は `liveProfileLabel` と同じ形）。トーストにしないのは、消えた後に
  「なぜ汚いのか」を知る手段が無くなるため。**原因を断定しない**（「回線が細い」とは
  書かない。測っていない）。計器が `aria-live` を持たないのに対し、この 1 行だけは
  `aria-live="polite"` を持つ（毎秒変わる値ではないので読み上げてよい）
- **通知は「その再生で下げた」ときだけ出す。** 画質そのものはチャンネルを
  切り替えても保つが（「この端末の回線」の性質）、別のチャンネルで
  「映像が止まったため下げました」と言うのは事実と食い違う。下げた再生の同定を
  一緒に持つ（`playingKey`）
- **成功した一覧をまだ持っていないとき（未着・初回の取得失敗）は `'wait'` を返す。**
  判定はクエリの状態（`isError`）ではなく一覧の有無で行う。TanStack Query は
  再取得に失敗しても前回の一覧を残す。`isError` で保留にすると、手元に一覧が
  あるのに下げなくなる（`pages/live.test.tsx` の「再取得が失敗しても」で固定）。
  「下げ先が無い」と同じ `false` にすると、hls.js 経路の観測は一度判定すると
  止まるので**その再生では二度と試さない**（`StallHandling`）。**ネイティブ経路の
  `'wait'` は現行どおりエラー文言に落ちる** --- あちらの猶予は `stalled` /
  `waiting` でしか張り直せず、無応答の配信ではそのイベントが再発火しないので、
  待つと黒いまま何も出なくなる（実測: `stalled` のあと 20 秒待っても来ない）
- **自動で下がった段と同じ値を選んでも自動は止まらない**（`change` が発火しない）。
  別の段を選べば止まる。セレクタに「自動」を常設しない判断の帰結である
  （停滞が一度も起きないデプロイに UI を 1 行足さない）

**判定手段**は 3 層ある。判定の本体（閾値との比較・`paused` / 非表示タブの抑止）は
`observeStall` として `lib/live.ts` にある。**実時間を使わずに** `lib/live.test.ts`
で見る（境界は 11,999 ms / 12,000 ms をリテラルで書く）。下げ先
（`nextLowerProfile`）と master 判定も同じ層である。
配線は `live-player.test.tsx` で見る。
hls.js 経路の 12 秒判定、`paused` の抑止、非表示タブでの基準の破棄、
`currentTime` が進んでいる間は数えないこと、master の抑止を含む。
引き取られたときはエラーにせず、引き取られなくても hls.js 経路はエラーにしない。
`pages/live.test.tsx` は**両方向**を見る。
自動のときは下げて通知を出し、`?profile=hd` のときは下げない。
下げ先が無いときはネイティブのエラー表示に落ちること（`live.profiles` が 1 件の
デプロイがこの経路に必ず入る）も見る。
実ブラウザは `web/e2e/live.mjs` の ⑪で見る。
**実測（実バイナリ + 実ブラウザ）**: 実再生させてから配信を止めると 22〜23 秒で
sd のプレイリストを取りに行き（12 秒の閾値 + 停止までの消費 + ポーリング）、
通知とセレクタが下がった先に一致し、離脱ヒントは飛ばない。
配信を復旧させると**そのまま再生が続く**（`currentTime` が進み `paused=false`）。
明示選択（`?profile=hd`）は同じ操作でも下がらない。
**⑪ は直す前の実装で落ちることを確認済みである**（NG 3 件）。
**降格のあとの再開を外した版でも、⑪ の「復旧後に再生が続く」が落ちることを
確認済みである**（対照の明示選択側は緑のままなので、切替に帰せる）。

**この降格は ⑦ の猶予を伸ばす。** WebKit でセグメントが応答しない場合、猶予の
満了で 1 回降格を試み、その先でまた停滞して初めてエラー表示になる
（**実測 31.5 秒**。降格の導入前は 30 秒以内に収まっていた）。「永久に黒いままに
ならない」という ⑦ の主張は変わらないが、窓は 40 秒にしてある（窓そのものが
「黒いままにならない」の上限である）。**降格は段が尽きるまで繰り返す**ので、
...
再生は `canplay` を待って再開する（上の「降格のあとは再生を再開する」を参照）。
**字幕は hls.js が新しいマニフェストを読むと選択を既定に戻すため**に戻す。
`SubtitleTrackController.onManifestLoading` が `trackId` を `-1` に戻す
（`node_modules/hls.js` 1.7.1 で確認済み）。そこで `MANIFEST_PARSED` で
`subtitleDisplay` を設定し直す。**その時点では既定トラックはまだ選ばれていない**
（同コントローラの `onManifestParsed` はトラック一覧を入れるだけ）ので、この設定は
**後の選択のときに `toggleTrackModes` が読む値**として効く。ネイティブ経路は
`loadedmetadata` でトラックの mode を揃え直す。この因果は実ブラウザで未検証である
（判定手段がまだ無い。下記）。

**音量とミュートは持ち越す必要が無い（実測）。** cleanup は
`removeAttribute('src')` + `load()` を行うが、`load()` は音量・ミュートを既定に
戻さない。**Chromium と WebKit の両方で実測した。** 復元のコードを外したビルドでも、
切替後に `video.volume` と `video.muted` が保たれる（`web/e2e/live.mjs` の ⑨）。

したがって ⑨ が守っているのは**切替で `LivePlayer` を作り直さないこと**の側である。
`key={selectedProfile}` を足す変異で音量・ミュートの両方が失われて ⑨ が落ちることを
確認済み。

**判定は `web/e2e/live.mjs` の ⑨ にある**（実ブラウザ + 実バイナリ）。jsdom では
原理的に測れない 2 点 —— 一覧が遅れて届いてもプレイリストを取り直さないこと、
切替で `<video>` が作り直されないこと —— を含む。⑨ は
`key={selectedProfile}` の変異で 4 件の NG（遅延到着での取り直し・離脱ヒントが
飛ぶ・音量が失われる・ミュートが失われる）を出すことを確認してある。

**字幕の持ち越しも実ブラウザで実測してある。** フィクスチャに字幕レンディション
（master playlist + WebVTT）を持たせ、`live.mjs` の ⑩ が 2 方向を見る。

- **hls.js 経路（Chromium）**: 利用者が切ってから画質を切り替えると `mode` は
  `hidden` のまま（再表示されない）。上の `MANIFEST_PARSED` の 1 行を外すと
  `showing` に戻る
- **ネイティブ経路（WebKit）**: 利用者が入にしてから切り替えると `showing` の
  まま。**`loadedmetadata` で揃えるだけでは足りなかった** —— 実測で `disabled` に
  戻った（WebKit は `loadedmetadata` の時点でまだトラックを作っていない）。
  そこで `TextTrackList` の `addtrack` でも適用している

**WebKit の字幕は既定で非表示である**（レンディションに `DEFAULT=YES` を書いても
Safari は利用者が有効にするまで出さない。実測）。したがって 2 方向を見ている ——
hls.js 経路は「切ったまま」、ネイティブ経路は「入にしたまま」。
...
出る。**`error` は即時、`stalled` / `waiting` は `liveStallTimeoutMs`（12 秒）の
猶予つき**にする。猶予の根拠は、`stalled` が出るのが途絶から 3 秒後、セグメント長が
2 秒なので、正常なら 3 セグメント以上落ちないと到達しないことである。
猶予中に `playing` / `canplay` /
`timeupdate` / `pause` が来たら回復と見なして捨てる。**猶予が満了したときは、
`error` と違って即座にエラーにせず、まず画質の自動降格を試す**（下記「停滞したら
画質を 1 段下げる」）。hls.js 経路にはこの watcher を張らない
...
`web/e2e/live.mjs` の ⑨（実ブラウザ + 実バイナリ）は画質切替を担う。見るのは
4 点である。一覧が遅れて届いてもプレイリストを取り直さないこと。切替で
`<video>` が作り直されないこと（音量・ミュートが保たれること）。切替が
離脱ヒントを送らないこと。`?profile=` が実際に要求に載ること。
...
「いま放送中」は既存の EPG API に時間窓を投げて出している（`lib/live.ts` の
`currentProgramWindow`）。**これは予測であって観測ではない**ので、画面でもそう見せる ---
番組名を断言せず、予定であることが読み取れる形にする。

**放送波を正とした「今の番組」は持たない。** 真実は EIT[p/f] にしかなく、読むには
多重化波ごとに恒久専有チューナーを払う（[../recording/delegation.md](../recording/delegation.md)
「onair tracker は採らない」）。ここで番組名は装飾で、外れてもユーザーは映っているものを
見ている --- 掛け金が合わない。番組名が外れて実害が出るのは録画側で、そこは終了だけ
mirakc が追従している（同「PSI/SI 追従」）。

**嘘の窓は Rokuban 側では縮まない。** `epg_programs` は mirakc の EIT[schedule] の射影である。
その鮮度は mirakc の `jobs.update-schedules` の間隔で決まる（既定は 1 日 2 回なので、
最大 12 時間前のスナップショット）。放送局が繰り下げを EIT[schedule] に出しても、その窓の
中では拾えない。Rokuban 側の EPG 取得頻度を上げても上界は動かないので、縮めたいなら
mirakc の設定を触る。

**観測を持つようになったら、予定と同じ列・同じフィールドに載せない。** 予定は毎パス
作り直せる導出値、観測は「誰が・いつ観測したか」が付いて回る事実で、寿命が違う
（不変条件 9 / 12）。置き場所は `(site, service_id)` を主キーにした揮発表で、
`epg_programs` にも `recordings` にも列は足さない。書くのは worker で、api は DB だけを
読む（不変条件 1）。mirakc 由来の `program_id` は保存しない（不変条件 7）。
