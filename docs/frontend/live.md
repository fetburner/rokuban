...
- **クライアント側の取り直しは `lib/events.ts` の専用グループ（`tuners`）に
  ...
  形で、周期も storage と同じ値（`storageRefreshIntervalMs`）を流用する。
  クエリキーは URL ではなく手書きの `['/api/tuners', site]` にする ---
  `/api/sites/{site}/tuners` のままだと `epg` グループの接頭辞（`/api/sites/`）
  にも一致するため。周期タイマーは周期の値ごとに別の `setInterval` に分かれて
  いるので、周期の違う 2 グループに同じキーが入ると、両方が発火する時刻
  （5 分と 10 分なら 10 分ごと）に同じクエリが 2 回取り直される
