...
**切替を跨いで音量・ミュート・字幕表示を持ち越す。** `LivePlayer` が effect の
cleanup で `<video>` から読み、次の setup で戻す。**字幕だけは作り直しで失われる。**
hls.js は新しいマニフェストを読むと字幕トラックの選択を既定に戻すためである。
...
`loadedmetadata` でトラックの mode を揃え直す。

**この持ち越しは実ブラウザで未検証である。** jsdom で測れるのは「どのイベントに
どの値を渡すか」の配線だけである。`<video>` の `load()` が音量・ミュートを保つか
（jsdom の `load()` は no-op）、Safari がトラックをいつ作るか、既定トラックが
`MANIFEST_PARSED` の時点で既に選ばれているか。これらは実機でしか確定しない。
外れた場合の帰結は「切替で音量・字幕が既定に戻る」で、配信そのものは壊れない。
