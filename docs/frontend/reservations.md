...
- **バッジは行本体の詳細リンクの中に置かない。** `<a>` の中に `<a>`（対話コンテンツの
  入れ子）はコンテンツモデル上不正でクリックの宛先が不定になる。予約一覧の行では、
  行本体の `Link`（詳細への導線）と容量バッジの `Link`（番組表への導線）を同じ `<li>`
  の兄弟要素にする
- **読み上げの規律は変えていない。** `<a>` の accessible name は `aria-hidden` な子を
  無視し `sr-only` な子は含めて計算されるので、外側を `span` から `Link` に替えても
  計算される名前（時刻を主語にした文）は変わらない
