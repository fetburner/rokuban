> [frontend.md](../frontend.md) の一部。索引から辿る

# デザイン言語（副調整室の計器盤）

**この節は以後の UI PR のレビュー判定基準。** 新しい画面・コンポーネントを足すときは
ここを満たしているかで見る。

形（トークン名と実際の値）の権威は `web/src/index.css`。ここには判断（なぜ）だけを書く。
**合否は実ブラウザで測る**（`pnpm e2e:design`。下記「合否は画素で測る」）。

## テーゼは「副調整室の計器盤」

Rokuban が扱うのは放送波と有限のチューナーで、UI の仕事は
**いま何が電波に乗っていて、何が録れていて、何が足りないか**を一目で言うこと。
装飾で個性を出す画面ではない。基準にするのは副調整室（サブ）の計器盤 ---
地は徹底して無彩、色が点いていること自体が情報。

原点は 16px のファビコン（[branding.md](branding.md)）で成立している階調構成で、
それをそのままアプリの地に持ち上げている。

## 地は「イ」の 3 値

地の階調は**ファビコンの 3 値と同じ値**を使う。

| ファビコン | トークン |
|---|---|
| 輝線 = 白 | `--paper`（紙白） |
| 間隙 = 灰 | `--scanline`（走査線グレー） |
| 字 = 黒 | `--sumi`（墨） |

値そのものはここには書かない（`index.css` と `scripts/gen-favicon.mjs` に既に 2 度
書いてあり、3 つ目を作ると必ずどれかが古くなる）。

ライトは「白地に黒」（ファビコンそのもの）、ダークは「消灯した画面に輝線」
（役割を入れ替える。反転ではない）。3 値の間を埋める段（`--tone-*`）は、
3 値がちょうど 50 / 500 / 900 に一致する Tailwind zinc の ramp から採る ---
別の ramp を持ち込むと、3 値だけが浮いて中間段と噛み合わなくなる。

zinc はわずかに寒色（hue ≈ 286、chroma ≤ 0.016）で、これが「地は無彩、色は信号」を
成立させている: 地が寒色側に寄っているぶん、暖色の信号（タリーレッド・琥珀）だけが
色として立ち上がる。

**3 値の一致は `web/tests/design-tokens.test.ts` が両方のファイルを読んで検査する**。
同じ決定が 2 箇所（`index.css` と `scripts/gen-favicon.mjs`）に書いてあるので、
片方だけ動かすと黙って食い違う。

例外は 1 つだけ。**浮いた面（`--card` / `--popover`）はライトで純白**を使う ---
ポップオーバー（録画の絞り込み・チャンネル選択）はオーバーレイを持たないので、
地と同色だと輪郭が罫線と影しか無くなる。

## 色は信号のみ

**恒常的な装飾色を持たない。** チャンネル色・アバター色・ブランドアクセントは入れない。
色が点いている箇所は、そのとき何かを主張している箇所に限る。

| 信号 | トークン | 意味 | 形 |
|---|---|---|---|
| タリーレッド | `--tally` / `--tally-foreground` | いま電波に乗っている（録画中・現在時刻） | **塗りと線のみ**（点灯した札・playhead） |
| 琥珀 | `--warning` | 警告（チューナー不足・条件なしのルール・期間指定の副作用） | 文字 + 淡い地 |
| destructive | `--destructive` | 取り返しがつかない・壊れた（失敗・ドロップ・EPG から消失・削除操作） | 文字 + 淡い地 |
| ジャンル淡色 | （Tailwind の 50 / 950） | 番組の種別。読まずに帯の並びを掴むための補助 | セルの淡い地 |

番組表のジャンル凡例は色と語の対応表であり、色だけに意味を持たせず再認を助ける。
放送終了セルの減光は状態色を増やさず、ジャンル淡色に muted の面を重ねる濃淡の変更とする。

**タリーは塗りと線にしか使わない。文字色にはしない。** 11px〜14px の赤い文字は
ダークの地に対して AA（4.5）に届かない。塗りにすれば「紙白の文字 / タリーの塗り」の
比で測れて、両テーマとも 4.5 を超える。この規律のおかげで「点灯している」という
比喩と読みやすさが同じ方向を向く。

**リストの「放送中」には色を使わない**（太さだけ）。チャンネル数ぶんの行が同時に
放送中になるので、赤くすると信号ではなく地になる。「いま」を主張するのは
番組表グリッドの線と札（画面に 1 本 / 1 枚しか出ない）。

`--tally` にだけ `--tally-foreground`（塗りの上に載る文字）を持たせ、`--warning` には
対の前景色を作っていない。琥珀は塗りに使わないので書き手がいない ---
使う人がいないトークンは足さない（CLAUDE.md 不変条件 11）。琥珀を塗りに使いたく
なったら、そのコードと同じ PR で `--warning-foreground` を足す。

**タリーレッドと destructive は色相が近い**。**区別は色ではなく形が担う**。
タリーは「点灯」なので塗り（`bg-tally` + 紙白の文字）、destructive は「壊れた」なので
文字と淡い地（`text-destructive` + `bg-destructive/10`）。同じ赤でも、
塗られているかどうかで「いま動いている」と「壊れた」が見分けられる。

この排他性のために外した色が 1 つある。**日付ストリップの「日 = 赤 / 土 = 青」は使わない。**
カレンダーの慣習ではあるが、赤が画面の中で 2 つの意味を持つと、
タリーが「いま電波に乗っている」を主張できなくなる。週末は濃さで立てる
（墨 = 週末 / 走査線グレー = 平日）。土と日のどちらかは文字自身が言うので、
色で区別する必要はない。

**沈黙は保証ではない。** 警告を出さないことは「大丈夫」を意味しない
（[data.md](../data.md) §6.5）。「競合なし」「収まります」を緑で出すことはしない ---
判定の盲点はすべて「警告を見逃す」方向に偏っている。緑の信号色を持たないのはこのため。

### コントラストは毎回測る

**数値はここに転記しない。** `pnpm e2e:design` が実画素から WCAG 比を計算して表で出し、
下限を割ったら落ちる。文字は 4.5、面と線は 3 が下限。

測る対象は「実際に出ている組み合わせ」で、**半透明の地は合成してから測る** ---
`text-warning` が乗るのは地ではなく `bg-warning/10` の上なので、地に対する比だけを
見ると 0.5〜0.7 ほど甘い数字が出る。

**判定を足したことと、それが効いていることは別**。合成の実装を入れても、淡い地を
持つのが外側の要素で文字を持つのが内側の要素だと、locator が内側だけを掴んで背景が
透明になる。その結果**合成が恒等関数になる**（この状態では色を暗くした修正を
revert しても判定は緑のまま通った）。いまは「祖先を遡って最初に不透明な面に当たるまで重ねる」形
にし、**掴んだ要素の背景が透明ならそのこと自体で落とす**。

**実画素は `getComputedStyle` からは読めない。** トークンが oklch なので Chromium は
計算値も `oklch(...)` のまま返す（canvas の `fillStyle` も同じ）。1px 塗って
`getImageData` で採る。`background-image` に直接書いた色も読めないので、縞の色は
カスタムプロパティに出してある（下記「走査線は 3 箇所限定」）。

下限を割る組み合わせは除外せず、通常の失敗判定に入れる（`knownGaps` のような
除外リストは持たない）。失敗バッジはライトの `--destructive` の明度を下げて直した。
**色相は動かしていない** --- タリーと近いままで、彩度だけがその明度で sRGB 色域に
収まる上限まで連れて下がる（上記「タリーレッドと destructive は色相が近い」参照）。録画中との識別は、従来どおり「塗り + 紙白の文字」と「文字 + 淡い地」の
形の差を保つ。

一覧行の hover 中の副情報は `text-muted-foreground` を維持する。4 画面（録画一覧・予約一覧・
ホーム・番組リスト）の `hover:bg-muted/40` を最も近い薄さへ揃えて、通常時の本文 = foreground /
副情報 = muted の階層を崩さずに調整する。hover はライト／ダークの両テーマで実測する。
録画一覧の選択モードで選んだ行も同じ `bg-muted/40` に揃えてある --- こちらは常時
見える面（Lighthouse の監査対象）なので、hover と同じ理屈で `e2e:design` が実測する。

`bg-muted` + `text-muted-foreground` は、地・文字とも走査線グレー側の段を経由する
ためライトで 4.5 を割る。走査線グレーの値そのものは動かさず、**次に挙げる箇所を
`text-foreground` にして直した**。録画一覧の「完了」・取り込みバッジ・site タグ、
番組リストの sticky 日付見出し、番組表グリッドとチャンネルピッカーのリモコン番号
タグである。さらに予約一覧の `StateBadge`/`ReservationSkipBadge`、ルール一覧の
「無効」、ライブのチャンネル種別タグ・チャンネル一覧のリモコン番号タグ、
chip / day-strip の hover も直した。
foreground は地の無彩 3 値の一部で信号色ではないので「色は信号のみ」とは矛盾
しない。新しく `bg-muted` の小バッジを足すときはこの手を最初から使う。
chip（`components/ui/chip.tsx`）・day-strip（`components/day-strip.tsx`）は常時では
なく hover で `bg-muted` が乗る形である。Lighthouse は hover を測らないので合否の
対象ではないが、同じ組み合わせである以上揃えて `hover:text-foreground` を
対にしてある。

**上の列挙は `bg-muted` 系の面に muted の文字が乗る箇所の網羅ではない。** 網羅の権威は
`e2e:design` の出力の側に置く。そこに出ていない組み合わせは「測っていない」であって
「通っている」ではない。今回の行 hover は 4 画面で `text-muted-foreground` を
維持したまま面の濃さを揃えた。対象は `components/program-row.tsx` /
`pages/recordings.tsx` / `pages/reservations.tsx` / `pages/home.tsx` である。
代表として録画一覧の行を `e2e:design` が実際に hover して測る。いま分かっている残りは:

- **測ってあり、下限を満たす**: 録画詳細（`/recordings/$id`）の `bg-muted/30` の
  パネルに乗る説明文・`<dt>` 群・品質イベント（`RecordingDetail`）。一覧は
  インライン展開を持たないので、この面が出るのは詳細ページだけである。hover と違って**常時見えるので
  Lighthouse の監査対象**に入るため、`e2e:design` が同じパネルの `<dt>` を測っている
  （同じ面・同じトークン対なので説明文・品質イベントも同値）
- **測っていない（未検証）**: 検索の `RuleSourceBanner`（`components/rule-form.tsx`）の
  読み込み中の枝（`bg-muted/40`）である。ここではルールが解決した本体が
  `text-muted-foreground` を持たず、エラーの枝は `text-destructive` である。
  もう 1 つはエンコード対象プロファイルの行の hover である
  （`components/encode-settings-fields.tsx` の `hover:bg-muted/60`）。そこでは
  コンテナ名が `text-muted-foreground` である。どちらも一瞬 / hover 中しか出ないため
  判定に載せていない。**他の不透明度の実測値から外挿もしない** --- 判定を足すまでは
  「同じ手で直した」扱いにも「通っている」扱いにもしない。destructive 側にも
  同様に未測定の組み合わせがある: `Button variant="destructive"` の hover
  （`hover:bg-destructive/20`。基底の `/10` より濃い面）と、
  `circuit-breaker-banner.tsx` の理由文（`text-destructive/80` が
  `bg-destructive/10` に乗る。文字自身が半透明）

モバイル番組のチャンネルピッカートリガー（`bg-background` + `text-foreground`）は
上記のバッジ群とは別の組み合わせで、他所（地の無彩 3 値そのものの対）で測っている
のでここでは測っていない。「トリガーが読めない」という古い観測がどの要素を指して
いたかは同定できていない（未検証）。リモコン番号タグはトリガーの中ではなく、
開いたポップオーバーの選択肢（`ChannelOption`）側にある。ポップオーバーは
`keepMounted` を付けていないので、閉じている間は DOM に存在しない。

### 操作標的の寸法は基準を先に決めて実測する

形を直す前に基準を固定する（[不変条件 11](../invariants.md)）。操作標的は
`jsdom` の DOM 属性や Tailwind のクラス名から推測しない。`web/e2e/design.mjs` の
④-A が Playwright の実ブラウザで `getBoundingClientRect()` を読み、主要画面
（番組・検索・予約・録画・ルール・ライブ）の実際に表示された標的を列挙する。
対象は `button, a[href], [role="button"], [role="switch"], input, select, summary`。
`display:none` / `visibility:hidden` / 祖先の `overflow` で隠れたものは除外し、
行の操作列のように折りたたまれた要素を「見えない標的」として数えない。
容量不足バッジの `::before` のような見た目と当たり判定が異なる実装は、擬似要素の
実寸を hit 寸法へ加える。スキップリンクは未フォーカス時の `sr-only` を除外し、
Tab 後に通常サイズへ戻る既存判定で測る。

現在の基準は次のとおり。数値は物理端末の pt / dp ではなく、ブラウザの CSS px
である。

| ポインタ / 用途 | 最低寸法 | 根拠と適用 |
|---|---:|---|
| fine / coarse の全操作標的 | 24 × 24px | [WCAG 2.5.8 Minimum (AA)](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum)。実効 hit 寸法で判定し、24px 未満は spacing 例外で見逃さない |
| coarse で頻度の高い主操作 | 高さ 44px 以上 | 行の予約・取消・ライブ、日付セル、チャンネル候補、ライブのチャンネル一覧。日付セルの幅は狭幅で列を均等配分するため 24px 以上を保ち、高さを44pxにする |
| coarse のモバイル主ナビ | 高さ 56px | `min-h-14` で safe-area と合わせて 44px 以上を確保し、4 項目（「その他」を含む）の一等地を維持する |

この下限を全ボタンへ 44px で強制しない。Rokuban は管理画面として、fine 側の
ボタン・chip・入力・select を 24〜36px 程度の密度で使う。一方、行の主操作や
モバイルで親指が頻繁に触る面だけ、配置文法として 44px 高さを採用する。予約行の
右端列の幅や主操作の重なりは [reservations.md](reservations.md) の規則が権威である。

④-A は fine のデスクトップと coarse の 360px モバイルを実測し、各標的の visual / hit
寸法、標的間の最小エッジ間隔、意図的な重なり件数を出力する。間隔は実測して記録する
が、全面リンクと行内操作のような意図的な重なりがあるため、全画面へ一律の gap を
課さない。合否は、列挙漏れがないこと、実効寸法が 24px 下限を満たすこと、そして
44px を役割として決めた標的の個別契約を満たすことで決まる。トーストの action
（`size="sm"`）は 32px、close（`size="icon-sm"`）は 28px として undo トーストの導線も
判定に含める。

[WCAG 2.5.5 Enhanced (AAA)](https://www.w3.org/WAI/WCAG22/Understanding/target-size-enhanced)
の 44 × 44px は強い参考線だが、WCAG のレベルも適用範囲も 2.5.8 とは異なる。
[Apple のアクセシビリティ指針](https://developer.apple.com/design/human-interface-guidelines/accessibility)
の 44pt、[Material のアクセシビリティ指針](https://m1.material.io/usability/accessibility.html)
の 48dp も各プラットフォームの指針であり、CSS px へそのまま置き換えない。
これらを根拠に dense な全画面を拡大するのではなく、用途とポインタを分けて実測する。

## トークン外の生の色値を書かない

コンポーネントが書いてよいのは**意味を持つトークン名**だけ
（`bg-background` / `text-muted-foreground` / `text-warning` / `bg-tally`）。
`bg-amber-700` のような Tailwind 標準パレットの直指定と、`#rrggbb` / `rgb()` /
`oklch()` の生値は書かない。**色の定義は `web/src/index.css` の 1 箇所に集める。**

```sh
cd web && pnpm check:colors
```

CI の lint job でも回る。除外は理由込みで `web/scripts/check-colors.mjs` に並べてある
（トークンの定義 / ジャンル淡色 / 映像のレターボックスとダイアログの幕の黒）。
**「テーマの色ではないもの」だけが除外の対象**で、ライト・ダークで変わってはいけない
黒（映像が来ていない領域・背後を沈める幕）がこれに当たる。

検査が**見ていない**書き方も同じファイルに書いてある（動的なクラス名の合成・CSS の
名前付き色・3 桁の 16 進・`public/` の資産）。「`check:colors` が通った」を
「色をトークン外に書いていない」と読み替えない。

濃さを変えたいときも**別の色を作らず、同じトークンの不透明度を下げる**
（容量超過の帯は琥珀の塗りを薄くし、罫線を濃く残す）。帯のためだけの琥珀を作ると、
琥珀が 2 つになった時点で「琥珀 = 警告」が言えなくなる。**不透明度の実際の値は
コンポーネント側が持つ**（ここに書くと、下限を割って直したときに追随し損ねる）。

## 文字サイズは 4 段に揃える

`xs` はバッジ、`sm` はチャンネル・日時・尺などの副情報、`base` は本文と一覧の題名、
`lg` は見出しに使う。実際の寸法は `index.css` と Tailwind のスケールを権威とし、
この文書には転記しない。直値の小さい文字は、情報密度が構造そのものになる番組表グリッドに限る。

一覧を持つページの本文には共通の幅上限を置き、サイドバー側へ左寄せする。
中央寄せにすると全幅の sticky ヘッダと本文の左端がずれ、視線の起点が二つになるためである。
放送局を横に並べる番組表グリッドは横幅が情報量なので、この上限の対象外とする。

## 数字は tabular-nums

時刻・尺・サイズ・ドロップ数・PID は縦に並ぶので、等幅数字にしないと桁が揃わず
「並べて比べる」という計器盤の唯一の仕事ができない。

**`html` に 1 度だけ当てて全域に効かせる**。**コンポーネント側で個別に指定しない**。
触るたびに指定を思い出す必要をなくすためである。`web/tests/font-tokens.test.ts` が
「コンポーネント側に `tabular-nums` の個別指定が残っていないこと」を検査する。
残っていれば全域適用に統合した決定と矛盾するので、そこで落ちる。フォントと
tabular-nums の実効性（どの書体が実際に等幅を作るか）は
[stack.md](stack.md)「フォントは英数字と和文で 2 書体を使い分ける」にある。

## 走査線は 3 箇所限定

走査線のモチーフ（縞）を使ってよいのは**空状態・読み込み中・ON AIR** の 3 箇所だけ。
全面に敷くとキッチュに転ぶ。ここが「地は無彩」との境界で、
走査線は色ではなく**質感**として現れる。

形は `web/src/index.css` の `.scanlines`（空状態・読み込み中）と
`.tally-scanlines`（ON AIR）の 2 ユーティリティ。周期 2px（輝線 1px + 間隙 1px）は
[branding.md](branding.md)「走査線は 3 値で描く」のファビコンと同じ設計を流用する。

**「読み込み中」が指すのは共通の `Skeleton` / `ListSkeleton`（components/page.tsx）
だけである**。`ListSkeleton` は `role="status"` + sr-only の「読み込み中」を 1 リージョンに
つき 1 度だけ持つ（内側の `Skeleton` は装飾のみで `aria-hidden`）。一覧の初回読み込みは
すべてこれを経由している。

各画面が個別に持つ `<p>読み込み中…</p>` 文言は、一覧の初回読み込みではないため
個々に `role="status"` を付けるだけに留めている。該当箇所:

- 起動時のサイトレジストリ取得
- 絞り込みポップオーバーのチャンネル選択
- ルール条件のサービス選択
- 番組詳細の展開
- `?ruleId=` の由来バナー
- ドロップ統計テーブル
- ライブプレイヤーの読み込み中表示

一覧の続き取得（次ページの自動読み込み）だけは `role="status"` を付けていない。
IntersectionObserver でスクロールのたびに mount/unmount するため、他の 7 箇所
（起動時 1 回だけ）と違って連続スクロール中に読み上げが繰り返されてしまう

`ListSkeleton` は行が縦に並ぶ一覧の形（高さ固定の行を並べる）を前提にしている。
ポップオーバーの幅制約やチップの折り返し、単一行の展開詳細、列で構成される表には
この形が合わない

**ファビコンの 3 値をそのまま持ち込むと本文の AA を割る。** ファビコンは 16px の
1 文字で WCAG の対象ではないが、空状態・ON AIR は実際に読む文字が乗る。
「輝線・間隙・字が互いに異なる 3 値」という**構造**だけを流用する。
縞 2 色の具体的な値は、本文の文字色（4.5 下限）に対して十分なコントラストが
立つ段へ選び直してある（`.scanlines` のコメント参照）。値そのものはここに
書かない --- `index.css` が権威である。ON AIR は色は信号のみの例外として
タリーレッドの塗りを使ってよい（上記「色は信号のみ」）。片方の縞にはタリーそのもの
（録画中バッジと同じ、AA を満たす組み合わせ）を使う。もう片方は**色相・彩度を
変えずに明度だけ落として**沈めてある。色相を変える手段
（`color-mix(in oklch, ...)` を墨と混ぜる等）は、oklch の補間が色相を極座標の
最短弧で結ぶせいでタリーの色相から外れた第 5 の信号色（マゼンタ寄り）を
生みかねない。CSS Color 4 の相対カラー
構文（`oklch(from var(--tally) calc(l * ...) c h)`）で明度だけを操作すれば
色相・彩度は変数元と完全に同じ値のまま動かない。

**縞の 2 色（`--scan-gap` / `--scan-lit`）は「文字に対してコントラストが低い
側」を `--scan-gap` に割り当てる**。**「暗い側」ではない**。空状態はライト・
ダークで字の明暗が入れ替わる（「地は『イ』の 3 値」）ので、不利になる縞も
入れ替わる。ライトは字が暗いので、明るい側（間隙）が不利になる。ダークは字が
明るいので、明るい側（輝線相当）が不利になる。そのため `--scan-gap` は
ダークでは輝線側を指す。ON AIR は字（`--tally-foreground` = 紙白）が両テーマで
変わらないので、`--scan-gap` は常にタリーそのもの側を指す。

実装は縞の 2 色を `--scan-gap` と `--scan-lit` に分けてある。`--scan-gap` は
`background-color` で、`--scan-lit` は `background-image` の
`repeating-linear-gradient` が参照するカスタムプロパティである。
**`background-color` は `getComputedStyle` で
読めるが、`background-image` に直接書いた色は読めない** --- カスタム
プロパティに出しておけば、`e2e:design` がどちらの縞も個別に読める。**両方**を
文字色との AA で判定できる。**「どちらが最悪ケースか」を決め打って片方だけ
測ってはいけない** --- そう決め打つと、決め打ちが外れたときに判定がすり抜ける。
決め打ちが正しくても、測っていない側を文字と同じ色にする変異が入ったときも
同じである。これは「コントラストは毎回測る」の「判定を足したことと、それが
効いていることは別」と同じ形である。使う側は
文字色を必ず `text-foreground`（ON AIR は `text-tally-foreground`）にする。

アニメーションは付けていない。CRT の走査を模した動きを検討したが、
このアプリの他の状態色がどれも静止していることを理由に見送った。**走査線
そのものの動きは無い。ただし読み込み中（`Skeleton`）は走査線とは無関係に
`animate-pulse`（不透明度の明滅）を持つ**。

3 箇所以外に走査線クラス（`scanlines` / `tally-scanlines`）が無いことは
`web/tests/scanline-usage.test.ts` が固定する。`scanlines` は `index.css`
定義 + `components/page.tsx`、`tally-scanlines` は `index.css` 定義 +
`pages/live.tsx` だけに閉じていることを見る判定である。4 箇所目を足すと落ちる。
手で確認するなら（`tally-scanlines` は文字列として `scanlines` を含むので、
ヒットしたファイルを目で見て 3 箇所に収まっているかを判断する）:

```sh
grep -rln --include='*.ts' --include='*.tsx' --include='*.css' 'scanlines' web/src | grep -v '\.test\.'
```

## アニメーションは `prefers-reduced-motion` で縮退する

動きの発生源は `animate-pulse`（`Skeleton`）である。ほかに、
ポップオーバー/ダイアログの `slide-in-from-*` / `zoom-in-95` / `fade-in-0` と、
`Button` の押下フィードバック（`translate`）がある。他にも
あり得るが、洗い出しは網羅していない。個別のクラスへ毎回縮退を書き足すのではなく、
`html` の `tabular-nums` と同じ方針で集約する。集約先は `web/src/index.css` の
1 ブロック（`@media (prefers-reduced-motion: reduce)`）である。
`animation-duration` / `transition-duration` をほぼ 0 に切り詰める
（`animation: none` にはしない --- 理由は `index.css` のコメント参照）。
実ブラウザでの縮退・非縮退の両方向の確認は `web/e2e/design.mjs` が権威。

## トーストのキーボード到達性

短命トーストの action には Alt+T でキーボードからアクセスできる。Alt+T でフォーカスを
移すことで、フォーカス中にタイマーを止める既存の一時停止もキーボードから利用できる。

## 角は立てる

`--radius` は `0.25rem`。shadcn 既定の `0.625rem`（10px）は SaaS のカードの丸みで、
1 行 56px の密な一覧に対して過剰に柔らかい。計器のパネルとスイッチは角が立っている。
`--radius-*` の派生段（0.6〜2.6 倍）はこの値に追従する。

## 頻度 3 段

配置は実装順ではなく**触る頻度**で決める。

| 段 | 対象 | 置き場 |
|---|---|---|
| 一等地 | 番組・録画 | 最短の導線。モバイルは親指の届く位置 |
| 中間 | 予約・ライブ | 一等地の次 |
| 端 | 検索・ルール・破壊的操作 | 意図して探しに行く位置 |

単一世帯の運用なので、頻度の仮説は自分の行動で検証してよい。

**「端」に寄せる実装手段は overflow メニュー（`components/ui/dropdown-menu.tsx`）
である**。`@base-ui/react` の `Menu` を shadcn の作法でラップした標準部品で
ある。破壊的・稀な操作はこれに乗せ、開く/編集のような主操作とは同格に並べない。
新しい破壊的・稀な操作を行に足すときは、新しい overflow の作法を都度作らずこの
部品を再利用する。適用先は増減しうるので、ここには数え上げない --- 実際の
適用箇所は `dropdown-menu` の import 元を grep すれば分かる。

## 合否は画素で測る

**色は jsdom では測れない。** Tailwind のクラスは解決されないし、oklch も計算されない。
`pnpm test` が全部通っても、色については何の保証にもならない。判定手段は 2 つに分かれる。

| 手段 | 見るもの | CI |
|---|---|---|
| `pnpm e2e:design` | **実ブラウザの実画素**。主要画面 × ライト / ダーク × デスクトップ / モバイルのスクリーンショットと、状態色・地の無彩性・コントラストの機械判定 | 載せない |
| `pnpm check:colors` | トークン外の生の色値（実ブラウザ不要） | lint job |
| `pnpm test`（`tests/design-tokens.test.ts` ほか） | 実ブラウザ無しで文字列として言えることだけ ---「3 値がファビコンと一致」「地と字が 3 値を参照している」「信号色がライト・ダーク両方にある」「どのトークンのクラスが当たっているか」 | web-test job |

トーストが多行に折り返したときのメッセージと action / close の縦中央一致も、`web/e2e/design.mjs` の実ブラウザ判定で測る。

**変えた状態色は必ず `e2e:design` の判定に載せる。** 判定の外に 1 箇所でも置くと、
そこだけ既定値へ静かに戻っても全部緑のまま通る。いま載っているのは録画中バッジ・
失敗バッジ・チューナー不足バッジ・番組リストの時刻（色が付いていないこと）・
現在時刻の線と札・容量超過の帯・ルールの「条件なし」。

手順は [web/e2e/README.md](../../web/e2e/README.md) §デザイン。
**判定を足すときは、直す前の実装で実際に落ちることを確認する。**

**`e2e:design` は CI に無いので、手元で走らせて exit code を見るところまでやる**。
判定側が DOM の変化（バッジが `<span>` から `<a>` に変わる等）に追随できていないと、
locator が空を掴んで赤くなる。しかし誰も走らせなければ数マイルストーン気付かれない
（実際に起きた）。

`e2e/design.mjs` は API を `page.route` で丸ごと差し替えるので、mirakc も実チューナーも
DB も要らない。時刻も固定してあり、ショットの差分は実装の差分だけになる。

## ダークは OS 設定に追従する

ダークへの到達手段は `prefers-color-scheme` への自動追従とする。
明示的な切り替え UI は持たない（下記「個人化は localStorage だけに置く」）。

初回フレームだけライトになることを避けるため、`index.html` の inline script が React の起動前に
`html.dark` を付ける。起動後は `main.tsx` が OS 設定の `change` を購読して同じクラスを同期する。
`.dark` クラスの経路は、実ブラウザ判定（`e2e:design`）で使うために残す。

## 個人化は localStorage だけに置く

**利用者が変えられる設定は、すべて端末ごとの `localStorage` に置く。サーバーには置かない。**

Rokuban は利用者という概念を持たない（[api/deployment.md](../api/deployment.md) §認証）。
サーバーに置いても「世帯でひとつ」の値にしかならず、端末ごとの値にはならない。
そして**端末をまたいで一致していないと壊れる個人設定が、いまひとつも無い**。
スマホと PC でテーマや密度が違うのはむしろ正しい挙動なので、端末ごとで足りる。

サーバーへ移す条件は 1 つだけ。**端末間で食い違うと事実が壊れる項目**（視聴済みの印など、
「どちらの端末が正しいか」を利用者が判定できない値）が出たときに、そのとき初めて表を作る。
それは個人設定ではなく世帯グローバルな事実なので、利用者ごとの表にはならない。

| 対象 | キー | 備考 |
|---|---|---|
| テーマ | 持たない（OS 追従のみ） | 上記「ダークは OS 設定に追従する」 |
| サイドバーの畳み | `rokuban:sidebar:collapsed` | [shell.md](shell.md) |
| 再生位置 | `rokuban:playback:<録画 id>:<プロファイル>` | 録画ごと |
| 再生速度 | `rokuban:playback-rate` | **端末ごとに 1 つ**（VOD と追っかけで共有） |
| 録画一覧の表示形式（リスト / カード） | `rokuban:recordings:view` | 出す情報は同じで並べ方だけが変わる |
| 番組表の表示形式（リスト / 番組表） | `rokuban:programs:view` | URL の `view` が優先。モバイルは常にリスト |
| 番組表の時間軸の縮尺 | `rokuban:programs:grid-scale` | 120 / 240 / 480 px/時。URL には持たない |
| 最後の検索条件 | `rokuban:search:last` | URL が優先（下記） |

**再生速度を録画ごとに持たない**のは、速度が「この録画をどう見るか」ではなく「自分が
どう見るか」の好みだから。録画を変えるたびに 1 倍へ戻ると、毎回選び直しになる。

### メディア操作は標準 controls と重ねない

自前の操作 UI は、次のどれかを担う場合に置く。

- 再生中の `<video>` の状態だけでなく資源を切り替える（プロファイル、字幕、追っかけの
  offset、原本 TS の取得）
- アプリやサーバーの状態を動かす（ライブ視聴の開始など）
- 標準 controls にその操作が無いことを対象ブラウザで実測した

再生・一時停止、シーク、音量、再生速度、PiP は、標準 controls が提供するブラウザでは
そちらを使う。同じ値を自前 UI と標準 UI の両方から変更できると、別々の表示や状態に
なって利用者を迷わせる。設定の保存が Rokuban 側にあることは、自前の操作 UI を置く
理由にしない。たとえば再生速度は、標準 controls の `ratechange` を端末の
`localStorage` に保存し、新しい VOD / 追っかけの `<video>` に `defaultPlaybackRate` として
適用する。通常のライブ視聴は別扱いにし、未検証のネイティブ HLS 再生速度を適用しない。

2026-09-23 の Chromium 151 実測では、標準 controls の `⋮` メニューに Playback speed と
Picture in Picture があったため、録画プレーヤーから再生速度セレクトと PiP ボタンを
外した。ブラウザごとに標準バーの機能は異なる。Rokuban 独自の操作を追加する前に、対象
ブラウザで標準 controls に機能が無いことを実測する。画面に置かないキー操作は、標準
controls に無い追加ショートカットだけを残し、入力欄や `<video>` にフォーカスがある間は
処理しない（[recordings.md](recordings.md)、[live.md](live.md)）。

### 検索条件は URL が正、localStorage は URL が空のときだけ

押した検索の条件は `/search?cond=<JSON>` にも載る。共有・ブックマーク・リロードで
同じ結果に戻るのは URL の仕事で、これは個人化ではなく**資源の同定**だから
（`/recordings` の絞り込みと同じ判断）。

localStorage の「前回の条件」は、**URL が条件を持たないときの初期値にしか使わない。**
順序は `?ruleId=`（ルール編集。条件の正本はルール）> `?cond=` > localStorage > 空。
復元するのはフォームだけで、**開いただけでは検索しない** —— 押していない操作が
起きたのと同じになる。

URL に載せる値は openapi 由来のスキーマ（`api/zod.ts`）でまるごと検証し、壊れた条件は
条件なしとして開く。localStorage 側も**読むたびに同じスキーマで検証する**（保存した
あとに openapi が変わる）。

### 視聴状態（視聴済み・続きから）はここで決めていない

上記の「サーバーへ移す条件」に挙げた視聴済みの印は、この節の対象外。世帯でひとつの
事実になりうる値で、個人設定ではない。**置き場所は未解決**で、別の判定基準タスクが
決める（追跡は open issue 側）。決まるまで「続きから」「未視聴」は実装しない。
**再生位置（`rokuban:playback:*`）はその判断より前から
localStorage にあるので、決定に合わせて移る可能性がある。**

### 設定ページは作らない

設定は「端」（上記「頻度 3 段」）だが、専用ページも作らない。**どの項目も、その画面を
見ているあいだにしか意味を持たない**ので、操作点そのものに置くほうが近い。
再生速度は VOD / 追っかけの標準 controls、表示形式は一覧のヘッダ、検索条件は検索フォーム
自身が持つ。

設定ページを作る条件は「**どの画面にも属さない項目**が出たとき」。それまでは、
1 画面 1 項目の設定を集めたページは、探しに行く手間を増やすだけになる。

### 持たないと決めたもの

- **番組表の局幅・フォントサイズの数値設定。** 列幅は画面幅への clamp で自動的に
  決まる（[programs.md](programs.md)）。時間軸の縮尺だけは、短い番組を正確に選ぶ
  判定基準として 3 段階を持つ（[programs.md](programs.md)）。
- **ボトムタブの項目カスタマイズ・並べ替え。** 頻度 3 段は設計の主張そのもので、
  並べ替えを渡すと主張が消える。単一世帯なので、頻度の仮説は自分の行動で検証できる
- **一覧の 1 ページ件数。** 一覧はページ番号を持たず、続きを継ぎ足す形（録画は
  スクロールでの追加取得、検索は「さらに表示」）なので、設定する対象が無い
- **行の高さ（密度）を表示形式とは別に持つこと。** リストとカードの 2 択で足りる。
  つまみを 2 つ渡すと、組み合わせの数だけ「誰も見ていない見た目」が増える

### localStorage の作法

- キーは `rokuban:` 接頭辞のあと関心事を先頭に置き、区別に要るぶんだけ `:` で
  スコープを足す（上の表がすべて）。例:
  - スコープ無し: `rokuban:playback-rate`
  - 2 段スコープ: `rokuban:playback:<録画 id>:<プロファイル>`
  - 画面:関心事: `rokuban:recordings:view`
- **読み書きは必ず try/catch で囲む。** private mode や設定でのストレージ無効化では
  参照そのものが例外を投げる
- **保存が無い状態が既定値。** `localStorage` を丸ごと消しても壊れる画面があってはならない。
  ストレージを触るテストは自分で後片付けする（`afterEach` で clear。
  `app-shell.test.tsx` だけは `beforeEach`。どちらでもよい）。
  そのうえで**既定値だけで成立するテストを書く** --- 片付けを忘れた隣のテストに
  汚されても落ちない状態にしておけば、順序に依存する壊れ方を持ち込まずに済む
- **初回フレームで見えてしまう値だけは `index.html` の inline script で React 起動前に当てる。**
  いまそこにあるのは `prefers-color-scheme` の判定だけ。地の色やレイアウトの密度を保存する
  ようになったらここに足すが、起動前に走るコードなので、初回フレームに映らない項目は足さない
