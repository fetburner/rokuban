...
--- foreground は地の無彩 3 値の一部で信号色ではないので「色は信号のみ」とは矛盾しな
い。新しく `bg-muted` の小バッジを足すときはこの手を最初から使う。

境界: 検索のルール条件見出し（`bg-muted/40` + `text-muted-foreground`）は同じ形では
ない --- 面が薄い（/40）ぶん比が高く出るので、判定に載せず `text-muted-foreground`
のままにしている。直すきっかけができたら（e.g. 実測が下限を割ったら）ここも同じ手で
直す。

モバイル番組のチャンネルピッカートリガー（`bg-background` + `text-foreground`）は
上記のバッジ群とは別の組み合わせで、実測ではライト 16.97・ダーク 19.06 と大きく
下限を超えている。「トリガーが読めない」という古い観測はいまの実装には当たらない
（実際に落ちていたのはトリガーの中のリモコン番号タグ、上の一覧の通り）。
