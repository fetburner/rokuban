...
段を経由するとライトで 4.5 を割る（録画一覧の「完了」・取り込みバッジ・番組リスト
の日付見出しがこの形だった）。走査線グレーの値そのものは動かさず、**これらの文字を
`text-foreground` にして直した** --- foreground は地の無彩 3 値の一部で信号色では
ないので「色は信号のみ」とは矛盾しない。予約一覧の「ルール外」（`StateBadge` の
detached）はまだ `bg-muted` + `text-muted-foreground` のままで AA に届いていないが、
判定にも載せていない。直すときは同じ手（`text-foreground` に変える）を使うか、
直さないなら `design.mjs` の判定に足して `knownGaps` に理由を書く。
