...
すべてこれを経由しており、各画面が個別に持つ `<p>読み込み中…</p>` 文言（起動時の
`site-gate.tsx`、絞り込みポップオーバーやルール条件のサービス選択、番組詳細の展開、
`?ruleId=` の由来バナー、一覧の続き取得、ドロップ統計テーブル、ライブプレイヤーの
読み込み中表示）は一覧の初回読み込みではなく個々に `role="status"` を付けるだけに
留めている。`ListSkeleton` は行が縦に並ぶ一覧の形（高さ固定の行を並べる）を前提に
しているため、ポップオーバーの幅制約やチップの折り返し、単一行の展開詳細、
列で構成される表には形が合わない
