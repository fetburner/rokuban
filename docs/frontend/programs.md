...
- **見えるラベルは局の列ではなく時間軸列（gutter）に出す。** 帯の内側に出すと帯の
  上端が番組セルの上端と重なったときセルの時刻文字と同じ px に描かれる（issue
  #460）。時間軸列は番組セルを一切置かないので、この列に出す限り衝突しない
- 見える文言は短く（「チューナー不足（BS が 1 本）」）、読み上げ用の文を `sr-only` に
  置いて見える側を `aria-hidden` にする（二重読みを避ける）。時間軸列の幅に収まら
  ない分は `truncate` で省略記号にする（読み上げ用の全文は `sr-only` 側が持つ）
