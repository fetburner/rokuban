...
- **削除 reconcile のサーキットブレーカー（`breaker.DeleteReconcile`）は site 列を持たない**（`internal/worker/delete_reconcile.go`）。値は空文字列で、他の 2 種のブレーカー名のように実際の束縛 site をキーにしない。`mirakcs:` に `default` という site が無い構成でも確実に解除できるよう、再開は site を含まない `POST /api/breakers/delete_reconcile/resume` を使う（詳細は [api/rest.md](api/rest.md)）
