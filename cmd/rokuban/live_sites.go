...
func (ls liveSites) Mount(r chi.Router) {
	...
	// variant / 字幕 playlist ルート（`/{name}`）。master は常に variant を相対 URI で
	// 指す。**ルート登録の可否を特定 site の設定に依存させない** --- `.vtt` の
	// 可否は dispatch 先の LiveStreamer.Segment が `cfg.Captions` を見て判定する。
