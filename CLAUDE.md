...
**docs を触ったら `npm run docs:lint` を回す**（textlint。CI の `docs-lint` ジョブと同じ）。全対象文書に同一プロファイル（1 文 120 文字の上限と読点 5 個の上限）を適用し、違反 0 件を gate にする（閾値では逃げられない）。対象は `README.md` / `CLAUDE.md` / `deploy/**/*.md` / `docs/**/*.md` / `web/e2e/**/*.md` の 5 グロブで、`package.json` の `docs:lint` が権威。**Go のテストや `deploy/k8s/` の検査を全部回しても、この 1 つだけは別に落ちる**（実際に PR を出してから気付いた）。
...
**`.github/workflows/ci.yml` の `generated-diff` ジョブが sqlc / oapi-codegen の版を固定してインストールする**。sqlc と oapi-codegen の生成物はヘッダにツール版を書く（`sqlc v1.31.1` / `version v2.8.0`）。そのためローカルの版がずれると差分が出て CI が偽陽性で赤くなる。**orval は 8.27.0 でヘッダから版を落とした**（`Generated by orval 🍺`）ので、ヘッダ由来の偽陽性は出ない。ただし出力自体は版で変わるので（8.22.0 → 8.27.0 で zod に `.int()` が付いた）、**orval を上げる PR では `web/src/api/` の 2 生成物を再生成してコミットする**。版を上げるときは `generated-diff` の pin とこの節（oapi-codegen の版）の両方を同じ PR で揃える。

**`web/package.json` の pnpm overrides には上限を書く**（`js-yaml: '>=4.3.0 <5'`）。advisory の下限だけを書くと、その後に出たメジャーへ黙って跳ね上がる。実際に orval が `js-yaml: 4.3.1` を固定していたのに、override が 5.4.1 を注入した。js-yaml 5 の ESM は default export を持たないので、`pnpm exec orval` が起動時に落ちた。
...
- **`reservations` と shadow-diff（#52 の出口基準を測る道具そのもの）を触るタスクは、#52 の並走中は着手しない。** 測定の連続性が切れる。並走が始まっているかは #52 を見て判断する（始まっていなければこの制約は効かない）
