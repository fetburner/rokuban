...
**`.github/workflows/ci.yml` の `generated-diff` ジョブが sqlc / oapi-codegen の版を固定してインストールする**。4 生成物はいずれもヘッダにツール版を書く（`sqlc v1.31.1` / `version v2.8.0` / `orval v8.22.0`。orval は API クライアントと zod スキーマの 2 つを出す）。そのためローカルの版がずれると差分が出て CI が偽陽性で赤くなる。版を上げるときは `generated-diff` の pin とこの節（oapi-codegen の版）の両方を同じ PR で揃える。
...
- **streamer のスケールとライブ視聴の資源同定は [docs/operations.md](docs/operations.md) §5「streamer のスケール」と [docs/api.md](docs/api.md) §ライブ視聴の HLS に決まっている**。sticky は使わない / ライブの URL にセッション ID を置かない / 既定 replicas=1 は可逆にする、の 3 点。触るタスク（#203）は実装前にこの 2 節を読む
...
毎パス再計算される値と、二度と再取得できない事実を 1 つの列に同居させると、**導出側が事実を上書きする**（実例 3 件は invariants.md §9）。混同は列だけでなく identity・式・適用の瞬間にも起きる。
...
- `WHERE a.x_id = b.id` を書くとき「**この id は誰が作り、いつ変わるか**」を問う。導出器が作るなら放送イベント `(site, network_id, service_id, event_id)` や `(site, program_id)` のように**外から与えられたキー**で引く。導出器が作るキーを保存する列は、読者を移設し終えたら DROP する（壊れ方 3 種は invariants.md §9「identity」）。
...
- **壊す場所を、実際に壊れる経路の上に置く。** 1 段下の関数を壊して落ちても、実物が通る経路を
  ...
  **CI 3 ジョブ緑のまま実バイナリが起動時 panic した**
- **実装の定数と比較するテストは何も主張していない。** `opts.Queue != cleanupQueue` は定数を
  変えても通る。期待値はリテラルで書く
- **CI が緑でも実バイナリ・実ブラウザを起動して確かめる。** テストが通っても起動しない／実
  クライアントから使えない類は、これでしか出ない。実例: 上記の起動時 panic / プレイリストの
  セグメント URI が basename で実 HLS クライアントから 404 / 実 Chrome が HLS の MIME に
  `'maybe'` を返すため全 Chrome ユーザーが再生できない
...
**正しさを落とさず、同じ問いを読み直す回数を減らす。** 詳細な sub issue はそのまま
spec / 実装計画として使い、判断や手順が欠けている場合だけ補う。同じ内容を別の設計書・
長い一時計画・task list に重ねて転記しない。

- **隔離 worktree のレビューへ未コミット差分を渡す前に patch を作る。** 隔離側からは
  元の working tree の未コミット差分が見えない。新規ファイルへ先に
  `git add -N <file>` を行い、`git diff <base>` を一時ファイルへ出して渡す。
  `git diff <base> --name-status` で新規ファイルを含む対象一覧も確認する
- **測定前に母集団を確認する。** 実環境の値が 0 件でも、受信方式・site・ロール等の一部
  だけを測った結果を製品全体へ一般化しない。規格や外部契約が形を決める問いは、ローカル
  測定より先にその一意性・許容範囲を確認する
- **実装中は targeted test、最後に full test とする。** red / green と変異確認は対象
  テストだけで回す。`go test ./...` / `pnpm test` / lint / build は完成差分に対して
  1 回実行する。
  レビュー修正後は変更箇所の targeted test を回し、実行時コード・契約・生成元を変えた
  場合だけ最後の full test を再実行する
- **生成は入力をまとめてから 1 回。** `openapi.yaml` / SQL の変更を確定してから
  oapi-codegen / orval / sqlc を再生成する。個々の編集ごとに生成しない
- **成功ログは要約する。** build のアセット一覧等は一時ファイルへ送り、成功時は終了状態と
  件数だけを残す。失敗時だけ診断に必要な箇所を読む。ただし失敗内容や既存 warning を
  隠して「成功」と報告してはならない
