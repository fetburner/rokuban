...
- **エージェントは明示的に必要なときだけ使う。** 読み取り・軽い調査・小さな編集は
  inline で行う。レビューは原則 1 体までとし、リポジトリ全体の自由探索ではなく、要件・
  対象ファイル・完成した diff を渡す。未コミット差分を隔離 worktree のエージェントに
  読ませるときは、先に `git add -N <新規ファイル>` を行い、`git diff <base>` を一時
  ファイルへ出して渡す。新規ファイルが diff から漏れていないか name-status でも確認する
...
- **成功ログは要約する。** build のアセット一覧等は一時ファイルへ送り、成功時は終了状態と
  ...
  隠して「成功」と報告してはならない
- **shell の cwd を毎回明示する。** `cd /absolute/path && ...` の形にし、別ターンの cwd に
  依存しない。特に `pnpm` をリポジトリ root で誤実行して不要な `node_modules` /
  lockfile を作らない
