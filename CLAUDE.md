...
**混同は列だけでなく identity と式にも起きる。** M2 完了後のレビューで見つかった 2 つの逃げ道:
...
|---|---|---|
| スキーマ v1「最終形で切る」（#13） | 不変条件 9 / 10 が M2-4 / M2-5 の後 | `reservations` を 5 本のマイグレーションで変更（`source` 削除 / `overrides` を 2 回移設 / チャンネル列追加 / dedup 列追加）。**churn を避けるための決定が最も churn した表を生んだ** |
