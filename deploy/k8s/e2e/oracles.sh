...
#
# **判定 2〜5 のオラクルは探索を身代わりに絞って回す**（E2E_FIXTURE_SCOPE）。
# 身代わりは製品と同じ役ラベル・同じキュー名を名乗るので、製品のワークロードが
# 入ると両方が一致して全部 AMBIGUOUS になり、**判定の有効性を確かめる唯一の
# 手段が、確かめたい時（ワークロードを足す PR）に使えなくなる**。オラクル 1 は
# 製品の api そのものが対象なので絞らない。
...
# 製品の api Service のセレクタを既定へ戻す（O1 の変異）。
#
# **これは fixture ラベルを持たないので preflight 0.5 では拾えない。**
# 戻し損ねると、以後の `run.sh` が「1.5 Service 経由で api に届かない」を
# 永久に FAIL し続け、preflight は何も言わない。
restore_api_service_selector() {
  k patch service rokuban-api --type=merge \
    -p '{"spec":{"selector":{"app.kubernetes.io/name":"rokuban","app.kubernetes.io/component":"api"}}}' \
    >/dev/null 2>&1 || true
}
...
oracle_teardown() {
  ...
  k delete jobs -l 'rokuban-e2e/fixture=true' --ignore-not-found >/dev/null 2>&1 || true
  # shellcheck disable=SC2086  # 空白区切りの一覧として展開させたい
  rm -f ${E2E_ORACLE_TMP:-}
  E2E_ORACLE_TMP=""
}

# oracle_tmp は後で消せる一時ファイルを作る。**mktemp を直接呼ばない** ---
# 中断すると 12 個が残る。
oracle_tmp() {
  local f
  f="$(mktemp)"
  E2E_ORACLE_TMP="${E2E_ORACLE_TMP:-} $f"
  printf '%s' "$f"
}
...
run_oracles() {
  # **中断でも片付ける。** Ctrl-C で teardown が走らないと、O1 が製品の
  # Service に当てた変異が残る（fixture ラベルが無いので preflight も拾えない）。
  E2E_ORACLE_TMP=""
  trap oracle_teardown EXIT
  ...
  # `--only` と同じ検証。ここが無いと `E2E_ORACLES_ONLY=9` が、クラスタを
  # 全部用意した後にオラクルを 1 本も回さず「走らせたものはすべて緑」を出す。
  if ! printf '%s' "$only" | grep -qE '^[1-5](,[1-5])*$'; then
    printf 'E2E_ORACLES_ONLY takes oracle numbers 1-5 (e.g. 2,4), got: %s\n' "$only" >&2
    return 64
  fi
  ...
  case ",$only," in *,2,*) with_fixture_scope oracle_check2 ;; esac
  case ",$only," in *,3,*) with_fixture_scope oracle_check3 ;; esac
