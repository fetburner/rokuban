...
//
// Queue は a.Site で修飾する（qualifyQueueName、issue #185 M4-13）。tuner_sync も
// 同じ epg キューを共有しているので、TunerSyncArgs.InsertOpts も同じ規則で修飾する
// （片方だけ修飾すると MaxWorkers: 1 による同時実行の抑制が site 単位に分かれて崩れる）。
func (a EpgSyncArgs) InsertOpts() river.InsertOpts {
	return river.InsertOpts{
		Queue: qualifyQueueName(epgQueue, a.Site),
