...
func (h *Server) PatchProgramOverrides(ctx context.Context, req PatchProgramOverridesRequestObject) (PatchProgramOverridesResponseObject, error) {
	...
	eff, err := reservation.EffectiveOptions(baseJSON, mergedJSON, nil)
	if err != nil {
...
func validateEffectiveKeepOriginal(eff reservation.ReservationOptions) error {
	...
	if eff.EncodeProfiles == nil || len(*eff.EncodeProfiles) == 0 {
		return fmt.Errorf(
			...
			reservation.KeepOriginalUntilEncoded, reservation.KeepOriginalAlways)
	}
	return nil
}
...
func parseProgramOverridesInput(in ProgramOverridesInput) ([]func(*reservation.ReservationOptions), []ProgramOverridesInputReset, error) {
	...
	if in.KeepOriginal != nil {
		...
		if v != reservation.KeepOriginalAlways && v != reservation.KeepOriginalUntilEncoded {
